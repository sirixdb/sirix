...
| C13 | β-survival check on a half that is itself a 1:31 lone *indirect* child | The lone child is an indirect node, so `discriminativeBits(half)` is well-defined and the β dispatch (C3/C4) applies — only a lone *leaf* child is C1. Two things do **not** carry over from a compressed half. (a) **Room**: a compressed half holds at most `MAX_NODE_ENTRIES - 1` children, so a fold always fits; a lone child was sized by its own inserts and may already be at `MAX_NODE_ENTRIES`. Decline the fold then (`FULL_EXISTING_BIT_LONE_HALF_FULL`) and let the complete structural frontier place K. A C2 collision adds no child and needs no room. (b) **Publication**: the half comes back as the node's *own* child reference, which already names the unfolded child in the transaction log or on disk. `registerFreshPage` stops at such a reference, so a folded page merely swizzled onto it is never logged — the writer and the commit both keep the unfolded child and K's leaf is lost with every invariant intact. Publish the folded page under a **fresh** reference and integrate the `BiNode` rebuilt around it (`FULL_EXISTING_BIT_LONE_HALF_FOLD`), never by re-pointing the half's reference. |
