...
versioning strategy and hash mode are likewise per-resource choices fixed in the
\code{ResourceConfiguration}. Secondary PATH, CAS, and NAME indexes have one implementation: HOT.
...
Two flags, \textbf{on by default} on the FILE\_CHANNEL backend, configure the low-latency
commit path: \code{sirix.commit.preallocated} and \code{sirix.commit.bufferedBeacons}. Setting
both to \code{false} restores the legacy grow-and-\code{force(true)} path (byte-identical for
resources that never ran preallocated; a resource that did keeps its zero tail as an unreachable
gap). The MEMORY\_MAPPED backend pins the legacy path regardless of the flags: its readers only
remap on physical file growth, which in-place preallocated commits never trigger.

Under preallocation, the data file is grown ahead of the write frontier in
\textbf{geometrically increasing chunks} (256~KiB floor, roughly doubling per grow to the
\code{sirix.commit.preallocChunkBytes} cap, default 8~MiB); the revisions file grows in fixed
64~KiB chunks (2{,}048 records) through its O\_DSYNC channel. A low-commit resource therefore
ends only a small chunk past its real data, with a fast first commit; a high-commit-rate
resource ramps to the cap and amortises the one-time per-chunk fsync over many commits. The
padding is physically allocated (zero-filled and fsynced — that is what keeps later in-place
writes journal-free) and persists across sessions by design.
...
\pp{io/sirix/index/hot/AbstractHOTIndexWriter.java}, \pp{HOTRangeCursor.java}.)}
...
Point lookup routes per level in \code{findChildByPartialKey}: compute the dense partial key
...
\section{HOT as the secondary-index structure}

Beyond intra-page navigation, HOT is the sole persistent structure for the secondary indexes of
Ch.~\ref{ch:pathsummary}. A generic \code{HOTIndexWriter<K>}
...
query:  jn:doc('db','catalog')//book          (all nodes named "book")
  ...
  NameFilter resolves "book" → name key 42
        │  HOTIndexReader performs one exact-key lookup
        ▼
...
chain. Every index uses the same canonical HOT representation from Ch.~\ref{ch:hot}; there is no
per-resource backend selector. A virgin index may be built in one sorted pass, while all later
insertions and removals mutate only the affected posting chunk in that same tree.

\begin{itemize}[leftmargin=1.4em]
\item \textbf{PATH}: primitive path-class key $\to$ chunked posting bitmap.
\item \textbf{CAS}: order-preserving $(path,type,value)$ key $\to$ chunked posting bitmap.
\item \textbf{NAME}: serialised qualified name $\to$ chunked posting bitmap.
\end{itemize}

\dcap{A secondary-index entry: key $\to$ posting list in the canonical HOT tree}
\begin{diagramS}
...
backing tree:
   HOT    Height-Optimised Trie over the serialised key (Ch. 5)

query: a bounded HOT cursor applies the typed/path predicate and emits NodeReferences
\end{diagramS}

At query time a HOT point or range cursor applies the declared \code{PathFilter},
\code{CASFilter}/\allowbreak\code{CASFilterRange}, or \code{NameFilter} and yields each surviving
entry's \code{NodeReferences}. A separate,
...
query:  for $b in jn:doc('db','store').book[] where $b.price eq 12.5 return $b
  ...
  CAS key = (12.5, double, PCR of that path)
        │  HOT point lookup seeks the exact serialised CAS key
        ▼
...
$\sim$20~KB leaf. These HOT slot values are the sole persisted secondary-index representation;
the unreachable alternate bitmap-chunk format has been removed.
...
and the projection listener) that the write transaction notifies on every insert/delete. Vector
embeddings have a separate explicit API because they cannot be derived from ordinary document
changes. The hot
...
the change to a key-local HOT posting insertion or removal. Listeners are re-created
...
\label{sec:vecstorage}

Two facilities exist. \textbf{PAX regions} (\pp{io/sirix/page/pax/}) group a parent key's children
into contiguous, SIMD-friendly columns \emph{within} a \code{KeyValueLeafPage} --- one column per
physical type, as a typed column store does it: integral (\code{NumberRegion}, frame-of-reference
{+} bit-packing), boolean (a bit column), string (\code{StringRegion}, per-page dictionary), and
fractional (\code{DoubleRegion}). Within the fractional region each field's column is encoded by the
field's \emph{declared} type, never by whether one value's binary image happens to round-trip:
a \textbf{double} column takes ALP (\code{ENC\_ALP}, with \code{ENC\_ALP\_RD} for the doubles ALP
cannot capture with few enough exceptions, and \code{ENC\_PLAIN} when neither pays), and an
\textbf{exact-decimal} column takes \code{ENC\_DEC}, which reuses the same block layout to carry the
values' own unscaled integers at a common scale. That second column is what makes real prices
scannable: only dyadic rationals equal their own double image, so \code{19.99} and \code{100.10}
have none, and a decimal column encoded over double images could not be compared soundly at all.
A field whose JSON numbers are mixed is therefore split across the integral and the fractional
column, and a page answers a predicate over it only when the typed columns \emph{together} account
for every occurrence of the field (\code{longCount}\,{+}\,\code{doubleCount} $=$ anchor slots);
otherwise the page goes through the records. \textbf{Projection-index leaf pages}
...
\noindent Across both facilities, whether a predicate can be answered \emph{in the encoded domain}
--- decided from the stored bytes, with no value ever materialised --- is a property of the
\emph{scheme}, not of the query. Frame-of-reference {+} bit-packing, ALP, and the exact-decimal
encoding are \textbf{order-preserving}: each is a monotonic transform, so a bound on the values
translates once per column into a bound on the stored integers and the comparison then runs over
packed bytes. The string dictionary and the boolean bit column are \textbf{identity-preserving}: an
equality is an equality over dictionary codes, and a flag column already \emph{is} the bitmap.
\code{ENC\_PLAIN} needs no transform at all, storing the doubles verbatim. \code{ENC\_ALP\_RD} is the
exception --- it is a \emph{bit split} of the double, not an arithmetic transform, so it is neither
order- nor identity-preserving --- and is therefore the one scheme whose predicate decodes, which is
a property of the scheme rather than a gap in the kernels.

\noindent The exact-decimal column additionally \textbf{refuses a bound expressed in double space}:
two distinct decimals can share a double image (\code{1000.25} and \code{1000.25000000000001} do), so
answering one from the other would silently put stored rows on the wrong side of the threshold. Its
bounds are converted by integer arithmetic at the column's own scale, which is what makes the
comparison exact rather than merely close. Refusal is \emph{enumerated} rather than a default arm: an
encoding no kernel recognises is refused explicitly instead of falling into a generic one, so an
unwired path is slow, never wrong.
...
framework gives the CAS/PATH/NAME indexes. No alternate secondary-index page format is retained:
projection chunks and posting lists share the canonical HOT persistence and versioning path.
\end{honesty}
...
one-sided comparison. Bit-packed widths up to $\sim$56 are gathered and unpacked, wider widths fall
to scalar.

\noindent \textbf{Composing predicates.} A kernel that returns a \emph{scalar count} cannot be
composed --- two counts cannot be AND-ed --- so every conjunction once had to be fused \emph{by
hand}, one kernel per predicate shape per encoding, and again for the liveness-masked variant. The
general route is now a \textbf{predicate tree evaluated into a selection bitmap}: each leaf answers
as a bitmap in its own encoded domain (a bit-packed range over the packed integers, a dictionary
equality over codes, the boolean column read as the bitmap it already is), and
\code{And}/\code{Or}/\code{Not} are word operations over those results. Composition therefore no
longer needs a kernel per shape per encoding, and a leaf shape the fused kernels never saw --- a
negation, a disjunction --- is served rather than refused. It also short-circuits: a conjunct that
selects nothing means the next field's column is never read, which a hand-fusion needing every leaf's
column up front cannot express.

Three things are deliberately \emph{kept} rather than subsumed. The zone-map-settled and
single-interval count kernels remain, because counting over packed integers without decoding does
strictly less work than building a bitmap and popcounting it --- and doing less work than
decode-and-filter is the entire point of the scan-over-compressed storage this chapter's columns
exist for. The hand-fused kernel remains as the \emph{fallback} for pages the tree route declines:
the route needs \code{RecordOrdinalRegion}'s record linkage (without it two columns' positions are
not the same rows) and refuses a non-zero alignment lead.
And the reach is \textbf{composition, not planning}: the tree is threaded to the scan alongside the
flattened \code{RegionCountPlan} (a closed union of shapes, and flattening is lossy) and is reached
only through one, so a predicate the planner cannot flatten at all still takes the record path.
Two selection currencies coexist on purpose --- index vectors \emph{between} operators, the shape
DuckDB and Umbra push across a pipeline, and bitmaps \emph{inside} one page's evaluation.

\noindent The \code{SirixVectorizedExecutor} fans scans across a worker pool (each
...
a sub-millisecond warm repeat), resolves the predicate's fields once per scan rather than per page,
and re-enters brackit's sequence model through a thin envelope
(\code{\{groupKey,count\}} arrays, scalar counts, or \code{[count,sum,min,max]}).
...
Unsupported cases \textbf{fail closed rather than risk a wrong answer}: \code{xs:float} operands
(the interpreter compares those in \emph{float} space, which double-space evaluation cannot
reproduce), string and boolean aggregation, and order-ambiguous group-by mixtures throw and defer to
the row-at-a-time interpreter. Decimals are \emph{not} in that list: a decimal predicate is compared
exactly --- over the exact-decimal column where the page has one, through the records otherwise ---
and a threshold whose own double image is inexact is served in decimal space rather than rounded
into double space. String \emph{predicate} matching is
...
Four further guards cover the bitmap route and the decimal column, each aimed at a way a
count-based test passes \emph{vacuously}. \code{DoubleRegionSelectEquivalenceTest} asserts
\code{popcount(select)} $=$ \code{count} \emph{per encoding} against brute force, because a
selection that is merely close is invisible to a count while putting rows on the wrong side of every
subsequent AND/OR. \code{PredicateBitmapEvaluatorTest} checks the algebra row-for-row against a
brute-force interpreter over 500 random trees. \code{PredicateTreeRouteTest} asserts the route is
actually \emph{reached} (via the per-page counter), because a path validated only by ``it agrees''
also agrees while it is dead code --- which this one was, until it became the primary route for
conjunctions. And \code{DecimalDoubleCollisionTest} pins the refusal of \S\ref{sec:vecstorage}: its
corpus holds decimals that share a double image, so an implementation answering a decimal tag from
double bounds returns a wrong count rather than a near-miss.
...
Secondary index point/range scan & $O(L+k)$ & HOT descent in key-byte length $L$ plus $k$ matches returned (Ch.~\ref{ch:hot}, Ch.~\ref{ch:pathsummary})\\
...
it is fail-closed to a small set of numeric, columnar query shapes, with string-predicate SIMD and
	non-\code{double} numeric columns still open. \textbf{The HOT index} is the sole PATH/CAS/NAME
	secondary-index representation. \textbf{The JSON FMES import} is library-only, not wired to a function, and its
...
secondary PATH/CAS/NAME       canonical HOT storage (not configurable)
...
FileChannel backend / commit profile
  ...
  -Dsirix.commit.preallocated       pre-grow the data file so a commit writes in place (force(false)); default true (FILE_CHANNEL only)
  -Dsirix.commit.preallocChunkBytes cap of the adaptive prealloc growth chunk (256 KiB floor, doubling; default cap 8 MiB)
  -Dsirix.commit.bufferedBeacons    buffer both beacons behind a single fsync (needs preallocated); default true
...
of, are below; the engine additionally defines internal kinds for path-summary nodes (\code{PATH}),
change tracking (\code{REVISION\_REFERENCES\_NODE}), the
...
tag   page kind           role
 ...
 12   HOTLeafPage         a secondary/projection/valid-time index leaf
...
[u8 pageKind][u8 V0][ body ]      Page-kind tags (1 byte; tag 7 is unused):
  1 KeyValueLeafPage   4 IndirectPage      8 CASPage       11 DeweyIDPage      14 reserved
...
CAS/Path/Projection/ValidTime [delegate][i32 count; count $\times$ (i32 indexId, i64 maxHotPageKey)]
...
The HOT-only secondary-index containers persist only their sparse physical-page allocators; their
delegate references root the HOT trees directly. Keyed-trie \code{maxNodeKey} and
\code{currentMaxLevels} metadata remains only on pages that actually own keyed tries. \code{NamePage}
additionally persists the five interned-name dictionaries
...
$\times$ 2 bytes \code{[dataLength:10 bits][kindId:6 bits]}, stored in big-endian byte order; (2) the
\textbf{offset-table template pool} (the
...
\textbf{Lowering commit latency.} The legacy commit pays a metadata-journal fsync on every file
growth. The preallocated profile (\code{sirix.commit.preallocated} with
\code{sirix.commit.bufferedBeacons}) pre-grows the file so a commit writes in place and skips the
journal fsync — the 1.29--1.51$\times$ commit-rate gain measured in Chapter~\ref{ch:eval} — and is
\textbf{on by default} on the FILE\_CHANNEL backend. Operators who monitor file-growth patterns, or
who run retried async-flush bulk imports (see \code{docs/KNOWN\_LIMITATIONS.md}), can restore the
legacy path with \code{-Dsirix.commit.preallocated=false}.
