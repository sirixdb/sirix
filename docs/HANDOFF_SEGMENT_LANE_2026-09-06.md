...
| q22 / q21 | 2.84 / 2.40 | 0.52, 0.38 / 0.021, 0.025 | LIKE predicates + `MIN(URL)`/`MIN(Title)`, `COUNT(DISTINCT UserID)` | **q22 rose to #5, q21 to #6.** Served since `a3aed07ec`, and the **advertised lever is already spent**: verdict sharing (`921c3f811`) and `MIN` over canonical collation ranks already serve both. Hot 100M profile: **q22 spends 68.9% of CPU in two predicate-mask evaluations; COUNT(DISTINCT) is 0.13%. q21 allocates about 1.49 GiB of canonical long lanes per hot try, with 99.2% of retained leaves empty.** See [evidence, empty-lane change and remaining target](SEGMENT_LIKE_GROUPS.md). Diagnostic results are unscored. |
