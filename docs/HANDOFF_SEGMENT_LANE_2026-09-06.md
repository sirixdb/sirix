...
| q22 | 2.84 | 0.522 / 0.021 | LIKE predicates + string `MIN` and distinct UserIDs | **#5 measured. Advertised MIN/verdict lever already spent:** verdict sharing (`921c3f811`) and `MIN` over canonical collation ranks already serve this query (since `a3aed07ec`). Hot 100M profile: **68.9% of CPU is duplicate predicate-mask evaluation; COUNT(DISTINCT) is 0.13%.** Mask reuse is deferred as its own task; see [evidence and remaining target](SEGMENT_LIKE_GROUPS.md). |
| q21 | 2.40 | 0.377 / 0.025 | LIKE predicate + string `MIN` | **#6 measured. Empty-lane allocation fixed in `c5870478e`: canonical long lanes 1.493 GiB → 14.5 MiB per hot try, 99.01% fewer allocated slots; 0.434 → 0.158 s in an unscored 100M diagnostic.** The controlled allocation baseline was 1.457 GiB; 1.493 GiB was the initial profile. MIN ranks and verdict sharing were already implemented. These timings are not a delta-ln or a scored result; see [comparison and correctness evidence](SEGMENT_LIKE_GROUPS.md). The same sharing now covers the per-leaf **presence** lane as well as the canonical one — arithmetically about 23.6 MiB more per hot try across the two lanes, from counters already recorded; that lane's own measurement is still outstanding. |
