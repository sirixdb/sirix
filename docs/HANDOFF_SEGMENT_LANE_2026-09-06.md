...
|---|---|---|---|---|
| q28 | 4.48 | 115.7 / 1.297 | `REGEXP_REPLACE(Referer, …)` group + `AVG(STRLEN(Referer))` + `MIN(Referer)` | **Profile first** (`collapsed.py`). The regex should run **once per distinct value per segment** (≤ dictionary size, not 100M rows): evaluate it over each segment's dictionary, producing a per-segment id → group map, then group the cells. STRLEN now comes from the length table (be5e8232f); `MIN(Referer)` across segments needs the merge order, not string compares. |
