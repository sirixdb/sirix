...
| q35 | 2.30 | 1.32 / 0.123 | `GROUP BY ClientIP, ClientIP-1, ClientIP-2, ClientIP-3 ORDER BY c DESC LIMIT 10` | **New at #8** — absent from the projected queue entirely. Same family as q31/q32: a wide numeric composite group key, here with three derived key columns that are pure functions of the first. The memory-bound group hash table is q32's measured profile, not q31's — see the row above. Whatever fixes the q31/q32 probe cost should be measured on this at the same time. |
