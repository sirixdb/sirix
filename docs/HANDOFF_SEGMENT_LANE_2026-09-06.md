...
acquisition, spill/copy — as the largest shared cost left on q32/q18/q16/q31. That profile justified reopening the composite table seam for the separate worker-probing study
below, superseding the earlier aggregate-family deprioritization for this measured cost. No
...
`order by … descending` placed the all-missing groups first — whose baseline proof is
[`SUM_ORDERING_DEFECT.md`](../bundles/sirix-query/bench/clickbench/rig/evidence/hicard-groupby-20260909/SUM_ORDERING_DEFECT.md).
**It is fixed** on `fm/sirix-sparse-sum-ordering-1` (`de268a8c0`), as its own wrong-results change
with its own review, ahead of the now-measured probing candidate: the in-kernel group comparator
had treated an all-missing group's `sum` as EMPTY, but `fn:sum(())` is 0 — the route itself emits
`"s":0` — so under `descending` those groups were placed empty-least. `sum` now always orders as a
value; `min`/`max`/`avg` keep empty placement. `SparseSumOrderingOriginTest` is the enabled
regression: the sparse fixture ordered by `count`, `sum` and `avg`, plus a mixed fixture where
`sum` under explicit `empty least` and `empty greatest` stays a value while `min`/`max`/`avg` keep
empty placement — every query byte-equal to the interpreter and served by the vectorized group
route. The 43-answer gate cannot see this case before or after the fix — no campaign query orders
on a sparse `SUM` — so that test, not any suite result, is its proof.

**2026-09-09, worker probing — measured family benefit, ordinary model resolved.** The
[completed study](../bundles/sirix-query/bench/clickbench/rig/evidence/composite-probe-20260909/RESULT.md)
rebased the parked prototype onto `2016aa8d3`, profiled that landed head, and compared candidate
`e8633fe92` over ten fixed pairs. All twenty legs passed 43/43 byte equality. q16/q18/q31/q32
benefit is **+0.181907 ln, 95% interval [+0.097559, +0.266254]**, saving **0.4716 seconds**.
The ordinary observed-effect requirement is ten pairs, completed; the conservative upper-noise
estimate is twelve and was not met. The fixed plan was not extended. q32 improves 3.5911 →
3.2748 s and q18 2.4205 → 2.3202 s; q16/q31 individual effects remain unresolved. The mechanism
is retained under the family landing rule. **Whole-suite score benefit -0.245162 ln remains
UNRESOLVED**, interval [-1.080341, +0.590016]. No publication leg or rank was added.

This completes the former first-ranked worker-probing candidate. Its earlier 0.721-ln working
payoff and 1.619-ln ceiling were hypotheses from the pre-budget profile, not measurements; the
observed family point estimate is about one quarter of that working payoff. The
[fresh baseline profile](../bundles/sirix-query/bench/clickbench/rig/evidence/composite-probe-20260909/PROFILE.md)
finds q32/q18/q16/q31 lookup at 46.1/38.3/31.4/18.4%, worker acquisition at
23.2/20.8/21.3/15.1%, and q32 exact merge/spill-copy at 22.8/21.6% (inclusive samples, with
overlap). q31's largest leaf is unpacking at 27.7%. A generic path that avoids intermediate
partition indexing and stripe copying alongside probes is a follow-up to investigate; it has
**no new payoff estimate or rank** yet. It must preserve bounded residency, duplicate-heavy
...
report delivery as pushed and locally validated, never CI green.
...
| q22 | 2.299 | 0.299 / 0.021 | LIKE predicates + string `MIN` and distinct UserIDs | **Its duplicate-mask cost is §4a row 1, the ranked predicate-mask lever. Advertised MIN/verdict lever already spent:** verdict sharing (`921c3f811`) and `MIN` over canonical collation ranks already serve this query (since `a3aed07ec`). Hot 100M profile (initial diagnostic): **68.9% of CPU is duplicate predicate-mask evaluation; COUNT(DISTINCT) is 0.13%** — the `b00ed9e4` capture re-reads the same shape at 68.8% and 0.23%. Its hot min(tries 2,3) read 0.278 s in the earlier capture and 0.316 s at `b00ed9e4` — a raw range across two unscored diagnostics, **no timing claim in either direction**. Mask reuse is deferred as its own task; see [evidence and remaining target](SEGMENT_LIKE_GROUPS.md). |
...
- **q31 windowed column read/decode (55 % CPU) and q32 grouping memory stalls.** Both shares were
  ...
  4.236 s. They describe a tree that no longer exists. The fresh landed-head profile at `2016aa8d3`
  ([`PROFILE.md`](../bundles/sirix-query/bench/clickbench/rig/evidence/composite-probe-20260909/PROFILE.md))
  supersedes them: q32 46.1% lookup and 21.6% spill/copy, q31 18.4% lookup and 27.7% unpacking.
  The worker-probing study above has measured that candidate; a further table/copy design is
  unranked until it has its own profile and payoff estimate.
