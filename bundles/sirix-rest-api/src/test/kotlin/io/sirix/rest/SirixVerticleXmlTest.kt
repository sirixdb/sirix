...
@DisplayName("XML Integration test")
        ...
        GlobalScope.launch(vertx.dispatcher()) {
            testContext.verifyCoroutine {
                ...
                var httpResponse = client.putAbs("$server$serverPath?hashType=ROLLING").putHeader(
                    ...
                    .putHeader(HttpHeaders.ACCEPT.toString(), "application/xml").sendBuffer(Buffer.buffer(xml)).coAwait()
                ...
                httpResponse =
                    client.postAbs(url).putHeader(
                        ...
                        .putHeader(HttpHeaders.ETAG.toString(), hashCode).sendBuffer(Buffer.buffer(xml)).coAwait()
                testContext.verify {
                    assertEquals(200, response.statusCode())
                    assertEquals(
