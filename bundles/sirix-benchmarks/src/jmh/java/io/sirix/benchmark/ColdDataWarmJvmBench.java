 ...
 *
 * <p>A JMH steady-state run has both warm — every record page is deserialized and resident, so a
 * route that avoids rebuilding pages saves nothing and its column read is pure addition. A fresh
 * JVM has both cold, so its number carries JIT warm-up and a first-touch of every page. Neither is
 * the shape a long-running server actually meets, which is a hot process reaching data that has
 * fallen out of cache.
 *
 * <p>Separating them needs the eviction to happen INSIDE the process: a second JVM would hand back
 * a cold JIT along with the cold data, which is the measurement that already exists.
 *
 * <h2>Protocol</h2>
 *
 * <ol>
 *   <li>Run the query {@code warmups} times, each with a DIFFERENT literal — the executor memoizes
 *       by (source path, predicate), so repeating one query measures a hash map. Different literals
 *       compile the same code and touch the same columns while leaving the memo cold.</li>
 ...
 *       than it can hold. {@code POSIX_FADV_DONTNEED} is not reachable from Java and
 *       {@code drop_caches} needs root, so cache pressure is what is left.</li>
 *   <li>Reopen and run the real query ONCE.</li>
 * </ol>
 *
 * <p>Whether step 3 worked is not assumed: SirixDB reports bytes served from the device versus from
...
public final class ColdDataWarmJvmBench {
  ...
  public static void main(final String... args) throws Exception {
    if (args.length < 6) {
      System.err.println("Usage: ColdDataWarmJvmBench <store> <db> <warmups> <evictGiB> "
                             + "<queryTemplate with %s> <literal>");
      System.exit(2);
    }
    ...
    for (int i = 0; i < warmups; i++) {
      ...
      System.out.printf("  warmup %d (%s): %8.1f ms  %s%n",
                        i, lit, (System.nanoTime() - t0) / 1e6, out.strip());
    }
    ...
    if (evictGiB > 0) {
      ...
      System.out.printf("  evicted: read %.1f GiB of unrelated data in %.1f s%n",
                        read / (double) (1 << 30), (System.nanoTime() - t0) / 1e9);
    }

    System.gc();
    final long t0 = System.nanoTime();
    final String out = runOnce(store, db, String.format(template, literal));
    System.out.printf("RESULT warm-jvm cold-data: %.1f ms  result=%s%n",
                      (System.nanoTime() - t0) / 1e6, out.strip());
  }
   ...
   */
  private static String runOnce(final Path store, final String db, final String query)
      throws Exception {
   ...
   *
   * <p>Everything in the store except the database under test, largest first, cycled until the
   * target is reached. Reading into a direct buffer that is thrown away each time keeps this from
   * turning into a heap benchmark of its own.
   *
   * @return bytes actually read
   */
  private static long evictPageCache(final Path store, final String db, final long targetBytes)
      throws Exception {
