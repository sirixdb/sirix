...
public final class JsonIntegrationTest extends AbstractJsonTest {
	...
	public void testSimpleRemove() throws IOException {
		final String storeQuery = """
				  jn:store('json-path1','mydoc.jn','[{"generic": 1}, {"location": {"state": "CA", "city": "Los Angeles"}}]')
				""";
		final String query = "let $doc := jn:doc('json-path1','mydoc.jn') for $i at $pos in $doc where deep-equal($i.generic, 1) return delete json $doc[$pos - 1]";
		final String openQuery = "jn:doc('json-path1','mydoc.jn')";
		final String assertion = """
	...
	public void testSimpleDeleteQuery() throws IOException {
		final String storeQuery = """
				  jn:store('json-path1','mydoc.jn','[{"generic": 1, "location": {"state": "CA", "city": "Los Angeles"}}]')
				""";
		final String query = "for $i in jn:doc('json-path1','mydoc.jn') where deep-equal($i.generic, 1) return delete json $i.location";
		final String openQuery = "jn:doc('json-path1','mydoc.jn')";
		final String assertion = """
				  [{"generic":1}]
				""".strip();
	...
	public void testTimeTravelQuery() throws IOException {
		...
		final String updateQuery2 = """
				...
				"""
				.strip();
		final String query = "let $node := sdb:select-item(jn:doc('json-path1','mydoc.jn'), 1) let $result := for $rev in jn:all-times($node) return if (not(exists(jn:previous($rev)))) then sdb:revision($rev) else if (sdb:hash($rev) ne sdb:hash(jn:previous($rev))) then sdb:revision($rev) else () return for $i in $result order by $i descending return $i";
		final String assertion = """
				  3 2 1
				""".strip();
	...
	public void testArray() throws IOException {
		final String storeQuery = """
				...
				""";
		final String query = "for $i in jn:doc('json-path1','mydoc.jn') where deep-equal($i.generic, 1) return {$i,'nodeKey': sdb:nodekey($i)}";
		final String assertion = """
	...
	public void testInsertObjectIntoObject() throws IOException {
		final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"bla\":true},{\"bar\":\"foobar\"},{\"bla\":null,\"foo\":false,\"baz\":null}]')";
		final String updateQuery = """
				  let $array := jn:doc('json-path1','mydoc.jn')
	...
	public void testArrayIteration() throws IOException {
		final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"key\":0,\"value\":true},{\"key\":\"hey\",\"value\":false}]')";
		final String openQuery = "for $i in jn:doc('json-path1','mydoc.jn') where deep-equal($i.key, 0) return { $i, \"nodekey\": sdb:nodekey($i) }";
		test(storeQuery, openQuery, "{\"key\":0,\"value\":true,\"nodekey\":2}");
	}

	@Test
	public void testNesting1() throws IOException {
		final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"key\":0},{\"value\":{\"key\":true}},{\"key\":\"hey\",\"value\":false}]')";
		final String openQuery = "for $i in jn:doc('json-path1','mydoc.jn')[].value where $i instance of object() and $i.key eq true() return { $i, \"nodekey\": sdb:nodekey($i) }";
	...
	public void testNesting2() throws IOException {
		final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"key\":0},{\"value\":[{\"key\":{\"boolean\":true}},{\"newkey\":\"yes\"}]},{\"key\":\"hey\",\"value\":false}]')";
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('//*', '//[]')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "for $i in jn:doc('json-path1','mydoc.jn')[].value[].key[?$$.boolean] return { $i, \"nodekey\": sdb:nodekey($i) }";
	...
	public void testNesting3() throws IOException {
		final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"key\":0},{\"value\":[{\"key\":{\"boolean\":5}},{\"newkey\":\"yes\"}]},{\"key\":\"hey\",\"value\":false}]')";
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:integer', '/[]/value/[]/key/boolean') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "for $i in jn:doc('json-path1','mydoc.jn')[1].value[].key[?$$.boolean gt 3] return { $i, \"nodekey\": sdb:nodekey($i) }";
	...
	public void testNesting4() throws IOException {
		...
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/statuses/[]/user/entities/url/urls/[]/url') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "for $i in jn:doc('json-path1','mydoc.jn').statuses[].user.entities.url[?$$.urls[].url eq 'https://t.co/TcEE6NS8nD'] order by sdb:nodekey($i) return {\"result\": $i, \"nodekey\": sdb:nodekey($i) }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting5() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/paths/\\/consolidated_screening_list\\/search/get/tags/[]', '/paths/\\/consolidated_screening_list\\/search/get/tags')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').paths.\"/consolidated_screening_list/search\".get.parameters return { \"result\": $result, \"nodekey\": sdb:nodekey($result) }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting6() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/paths/\\/consolidated_screening_list\\/search/get','/paths/\\/consolidated_screening_list\\/search/get/parameters/[]/name')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = """
	...
	public void testNesting7() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/paths/\\/consolidated_screening_list\\/search/get/parameters/[]/name') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').paths.\"/consolidated_screening_list/search\".get[?$$.parameters[].name = 'keyword'] return { \"result\": $result, \"nodekey\": sdb:nodekey($result) }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting8() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, '/paths/\\/consolidated_screening_list\\/search/get/parameters/[]/name') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').paths.\"/consolidated_screening_list/search\".get.parameters[3].name return { \"result\": $result }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting9() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, '/sirix/[]/revision/tada/[]/foo') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[1].revision.tada[0].foo return { \"result\": $result }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting10() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, '/sirix/[]/revision/tada') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[1].revision.tada[0] return { \"result\": $result }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting11() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, '/sirix/[]/revision/tada/[]') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[4] return { \"result\": $result }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting12() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/sirix/[]/revision/tada/[]/foo','/sirix/[]/revision/tada/[]/[]/foo')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[][].foo return $result";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting13() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[?$$[][].foo[].baz = 'bar'] return $result";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting14() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
	...
	public void testNesting16() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $baz := jn:doc('json-path1','mydoc.jn') let $return := $baz.sirix[2].revision.tada[4][0].foo[1].baz return $return";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting17() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/paths/\\/consolidated_screening_list\\/search/get')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = """
				for $i in jn:doc('json-path1','mydoc.jn').paths."/consolidated_screening_list/search".get
				let $j := $i.parameters[].name
				return for $k in $j
	...
	public void testNesting18() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/pathx')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = """
				for $i in jn:doc('json-path1','mydoc.jn').paths."/consolidated_screening_list/search".get
				let $j := $i.parameters[].name
				return for $k in $j
	...
	public void testNesting19() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('foo')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = """
	...
	public void testNesting20() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/paths/\\/consolidated_screening_list\\/search','/paths/\\/consolidated_screening_list\\/search/get/parameters/[]/name')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = """
				for $i in jn:doc('json-path1','mydoc.jn').paths."/consolidated_screening_list/search"
				let $j := $i.get
				let $l := $j.parameters[].name
	...
	public void testNesting21() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[?$$[][].foo[].baz >= 'baa' and $$[][].foo[].baz <= 'brr'] return $result";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting22() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-name-index($doc, 'foo') return {\"revision\": sdb:commit($doc)}";
	...
	public void testNesting23() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-name-index($doc, 'revision') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[].revision.foo return $result";
		test(storeQuery, indexQuery, openQuery,
				Files.readString(JSON_RESOURCE_PATH.resolve("testNesting22").resolve("expectedOutput")));
	}
	...
	public void testNesting24() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[][].foo[].baz[?starts-with($$, 'ba')] return $result";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting28() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[][].foo[].baz[] return $result";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting29() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision[?$$.tada[][].foo[1].baz = 'bar'] return $result";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting30() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, '/sirix/[]/revision/tada/[]') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[-1] return { \"result\": $result }";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNesting32() throws IOException {
		...
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/sirix/[]/revision/tada//[]/foo/[]/baz') return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "let $result := jn:doc('json-path1','mydoc.jn').sirix[2].revision.tada[4][?$$[].foo[1].baz = 'bar'] return $result";
		test(storeQuery, indexQuery, openQuery,
	...
	public void testNonExistentPath1() throws IOException {
		final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"key\":0},{\"value\":[{\"key\":{\"boolean\":true}},{\"newkey\":\"yes\"}]},{\"key\":\"hey\",\"value\":false}]')";
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('//*', '//[]')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "jn:doc('json-path1','mydoc.jn')[].value[].key[?$$.boolean].nonExistent";
		test(storeQuery, indexQuery, openQuery, "");
	}
	...
	public void testNonExistentPath2() throws IOException {
		final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"key\":0},{\"value\":[{\"key\":{\"boolean\":true}},{\"newkey\":\"yes\"}]},{\"key\":\"hey\",\"value\":false}]')";
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('//*', '//[]')) return {\"revision\": sdb:commit($doc)}";
		final String openQuery = "jn:doc('json-path1','mydoc.jn')[].value[].key.nonExistent";
		test(storeQuery, indexQuery, openQuery, "");
	}

	@Test
	public void testCreateAndScanNameIndex() throws IOException {
		final URI docUri = JSON_RESOURCE_PATH.resolve("testCreateAndScanNameIndex").resolve("multiple-revisions.json")
				.toUri();
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-name-index($doc, ('foo','bar')) return {\"revision\": sdb:commit($doc)}";
	...
	public void testCreateAndScanPathIndex() throws IOException {
		final URI docUri = JSON_RESOURCE_PATH.resolve("testCreateAndScanPathIndex").resolve("multiple-revisions.json")
				.toUri();
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = """
				let $doc := jn:doc('json-path1','mydoc.jn')
	...
	public void testCreateAndScanCASIndex() throws IOException {
		final URI docUri = JSON_RESOURCE_PATH.resolve("testCreateAndScanCASIndex").resolve("multiple-revisions.json")
				.toUri();
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = """
				let $doc := jn:doc('json-path1','mydoc.jn')
	...
	public void testCreateAndScanCASIndex2() throws IOException {
		final URI docUri = JSON_RESOURCE_PATH.resolve("testCreateAndScanCASIndex2").resolve("multiple-revisions.json")
				.toUri();
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = """
				let $doc := jn:doc('json-path1','mydoc.jn')
	...
	public void testCreateAndScanCASIndex3() throws IOException {
		final URI docUri = JSON_RESOURCE_PATH.resolve("testCreateAndScanCASIndex3").resolve("copperfield-book.json")
				.toUri();
		final String storeQuery = String.format("jn:load('json-path1','mydoc.jn','%s')", docUri);
		final String indexQuery = """
				let $doc := jn:doc('json-path1','mydoc.jn')
