...
/**
 * Integration tests that verify Path, CAS, and Name indexes correctly work
 * across multiple revisions. These tests verify that indexes improve query
 * performance and return correct results as the document evolves.
 * <p>
 * Note: Indexes in SirixDB are used as query optimizations. The query engine
 * automatically uses indexes when navigating paths that match indexed patterns.
 */
public final class IndexVersioningIntegrationTest extends AbstractJsonTest {
  ...
  class PathIndexTests {
    ...
    void testPathIndexAfterModification() throws IOException {
      ...
      query("let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/products', '/products/[]', '/products/[]/name')) return sdb:commit($doc)");
      query("append json {\"name\": \"Gadget\", \"price\": 20} into jn:doc('json-path1','mydoc.jn').products");
      test("for $p in jn:doc('json-path1','mydoc.jn').products[] order by $p.price return $p.name", "\"Widget\" \"Gadget\"");
    }

    @Test
    @DisplayName("Path index correctly filters with predicates")
    void testPathIndexWithPredicates() throws IOException {
      final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"id\":1,\"active\":true},{\"id\":2,\"active\":false},{\"id\":3,\"active\":true}]')";
      final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('//[]', '//*')) return {\"revision\": sdb:commit($doc)}";
      final String query = "for $i in jn:doc('json-path1','mydoc.jn')[] where $i.active eq true() order by $i.id return $i.id";
  ...
  class CASIndexTests {
    ...
    void testCASIndexStringAfterUpdate() throws IOException {
      query("jn:store('json-path1','mydoc.jn','[{\"status\":\"pending\"},{\"status\":\"pending\"},{\"status\":\"done\"}]')");
      query("let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/[]/status') return sdb:commit($doc)");
    ...
    void testCASIndexNumericOperations() throws IOException {
      final String storeQuery = "jn:store('json-path1','mydoc.jn','[{\"price\":10},{\"price\":50},{\"price\":100},{\"price\":200}]')";
      final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:integer', '/[]/price') return {\"revision\": sdb:commit($doc)}";
  ...
  class NameIndexTests {
    ...
    void testNameIndexFieldLookup() throws IOException {
      final String storeQuery = "jn:store('json-path1','mydoc.jn','{\"data\": {\"title\": \"First\"}, \"metadata\": {\"title\": \"Second\"}}')";
      final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-name-index($doc, ('title', 'data', 'metadata')) return {\"revision\": sdb:commit($doc)}";
    ...
    void testNameIndexAfterAddition() throws IOException {
      ...
      query("let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-name-index($doc, ('items', 'tag')) return sdb:commit($doc)");
  ...
  class CombinedIndexTests {
    ...
    void testMultipleIndexes() throws IOException {
      final String storeQuery = "jn:store('json-path1','mydoc.jn','{\"users\": [{\"name\": \"Alice\", \"age\": 30}, {\"name\": \"Bob\", \"age\": 25}]}')";
      final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $pathIdx := jn:create-path-index($doc, ('/users', '/users/[]')) let $casIdx := jn:create-cas-index($doc, 'xs:integer', '/users/[]/age') let $nameIdx := jn:create-name-index($doc, ('users', 'name', 'age')) return sdb:commit($doc)";
    ...
    void testIndexWithDeleteAndInsert() throws IOException {
      ...
      query("let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('//[]', '//*')) return sdb:commit($doc)");
    ...
    void testIndexWithReplace() throws IOException {
      ...
      query("let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-cas-index($doc, 'xs:string', '/item/name') return sdb:commit($doc)");
    ...
    void testIndexDeepNesting() throws IOException {
      final String storeQuery = "jn:store('json-path1','mydoc.jn','{\"l1\": {\"l2\": {\"l3\": [{\"val\": 1}, {\"val\": 2}]}}}')";
      final String indexQuery = "let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-path-index($doc, ('/l1', '/l1/l2', '/l1/l2/l3', '/l1/l2/l3/[]')) return sdb:commit($doc)";
  ...
  class MultiRevisionTests {
    ...
    void testNameIndexFieldAdditionsAcrossRevisions() throws IOException {
      ...
      query("let $doc := jn:doc('json-path1','mydoc.jn') let $stats := jn:create-name-index($doc, ('root', 'items', 'tag')) return sdb:commit($doc)");
