 ...
 * {@link SirixVectorizedExecutor#rewriteFpCmpForIntegralColumn(int, int, double)}: the rewrite of a
 * double-threshold comparison over a provably-integral NUMERIC_LONG projection column into an exact
 * long-space predicate.
 *
 * <p>
 * The specification is the interpreter's numeric promotion: a long {@code L} matches {@code <op> d}
 * iff {@code Double.compare((double) L, d) <op> 0}. Every rewrite is verified by brute force
 * against that oracle over adversarial candidate longs: values around the threshold, around
 * {@code ±2^53} (where {@code (double) L} starts rounding), and at the
 * {@code Long.MIN_VALUE}/{@code MAX_VALUE} extremes.
 */
public final class FpCmpIntegralRewriteTest {
  ...
  private static final double[] THRESHOLDS =
      {9.99, -9.99, 0.5, -0.5, 0.0, -0.0, 10.0, -10.0, 1.0, -1.0, 20.5, 20.999, 0.1, -0.1, 9007199254740991.0, // 2^53 -
                                                                                                               // 1
                                                                                                               // (exact)
          9007199254740992.0, // 2^53 (exact; 2^53 + 1 is NOT a double)
          9007199254740994.0, // 2^53 + 2
          -9007199254740992.0, 1.152921504606847E18, // 2^60 — multiple longs share one double image
          -1.152921504606847E18, 9.223372036854776E18, // (double) Long.MAX_VALUE == 2^63
  ...
  private static List<Long> candidatesFor(final double d) {
    ...
    final long[] anchors = {0L, 1L, -1L, Long.MIN_VALUE, Long.MAX_VALUE, (1L << 53) - 2, (1L << 53) - 1, 1L << 53,
        (1L << 53) + 1, (1L << 53) + 2, -(1L << 53) - 2, -(1L << 53) - 1, -(1L << 53), -(1L << 53) + 1, 1L << 60,
        (1L << 60) + 1, (1L << 60) - 1,};
  ...
  void rewriteMatchesOracleForAllOpsAndThresholds() {
    for (final double d : THRESHOLDS) {
      for (final int op : OPS) {
        final ProjectionIndexScan.ColumnPredicate p = SirixVectorizedExecutor.rewriteFpCmpForIntegralColumn(0, op, d);
        assertEquals(0, p.column);
        for (final long v : candidatesFor(d)) {
          assertEquals(oracle(v, op, d), evalRewritten(p, v),
              "op=" + op + " d=" + d + " v=" + v + " rewritten=" + p.op + "/" + p.longLit + (p.highLit != 0
                  ? "/" + p.highLit
                  : ""));
        }
      }
  ...
  void randomizedSweepAgainstOracle() {
    ...
    for (int t = 0; t < 200; t++) {
      ...
      for (final int op : OPS) {
        final ProjectionIndexScan.ColumnPredicate p = SirixVectorizedExecutor.rewriteFpCmpForIntegralColumn(0, op, d);
        for (final long v : candidatesFor(d)) {
          assertEquals(oracle(v, op, d), evalRewritten(p, v), "op=" + op + " d=" + d + " v=" + v);
        }
        for (int r = 0; r < 32; r++) {
  ...
  void fractionalEqualityIsUnsatisfiable() {
    final ProjectionIndexScan.ColumnPredicate p = SirixVectorizedExecutor.rewriteFpCmpForIntegralColumn(3, OP_EQ, 9.99);
  ...
  void hugeThresholdEqualityMatchesEveryLongSharingTheDoubleImage() {
    ...
    final ProjectionIndexScan.ColumnPredicate p = SirixVectorizedExecutor.rewriteFpCmpForIntegralColumn(0, OP_EQ, d);
