 ...
 *
 * <p>Every selected record has same-named decoys both above and below the selected JSON depth. The
...
final class NoPathSummarySourceScopeDifferentialTest {
  ...
  private record Snapshot(String count, String sum, String predicateCount, String predicateSum,
                          String groups, String predicateGroups, String distinct, String multiGroups) {
  }
  ...
  void exactSourceDepthSurvivesUpdatesDeletesInsertsAndHistory(final VersioningType versioning)
      throws Exception {
    ...
    final SourceCase[] sources = {
        new SourceCase("top-level", new String[] {"[]"}, new Expected[] {
            new Expected(2, 300, 1, 100, 2),
            new Expected(2, 310, 1, 110, 2),
            new Expected(2, 310, 1, 110, 2),
            new Expected(2, 310, 1, 110, 2),
            new Expected(2, 310, 1, 110, 2)
        }),
        new SourceCase("nested rows", new String[] {"[]", "rows", "[]"}, new Expected[] {
            new Expected(3, 60, 3, 40, 2),
            new Expected(3, 61, 3, 41, 3),
            new Expected(4, 101, 3, 81, 3),
            new Expected(4, 101, 3, 81, 3),
            new Expected(3, 81, 3, 81, 3)
        })
    };
  ...
  private void commitRevisionTwo(final VersioningType versioning) throws Exception {
    try (var store = newStore(versioning);
      ...
      update(chain, ctx, "for $o in " + latestSource("[]")
          + " where $o.outerId eq 1 return replace json value of $o.value with 110");
      update(chain, ctx, "for $o in " + latestSource("[]")
          + " where $o.outerId eq 1 return replace json value of $o.group with \"outer-c\"");
      update(chain, ctx, "for $r in " + latestSource("[].rows[]")
          + " where $r.id eq 1 return replace json value of $r.value with 11");
      update(chain, ctx, "for $r in " + latestSource("[].rows[]")
  ...
  private void commitRevisionThree(final VersioningType versioning) throws Exception {
    try (var store = newStore(versioning);
         ...
         var chain = SirixCompileChain.create()) {
      update(chain, ctx, "for $r in " + latestSource("[].rows[]")
          + " where $r.id eq 4 return insert json {\"value\":40} into $r");
      ctx.applyUpdates();
    }
  }
  ...
  private void commitRevisionFour(final VersioningType versioning) throws Exception {
    try (var store = newStore(versioning);
      ...
      update(chain, ctx, "for $o in " + latestSource("[]") + " where $o.outerId eq 1 return insert json "
          + "{\"lateNested\":{\"id\":905,\"value\":7000,\"group\":\"decoy-e\",\"active\":true}} "
          + "into $o");
      ctx.applyUpdates();
    }
  }
  ...
  private void commitRevisionFive(final VersioningType versioning) throws Exception {
    try (var store = newStore(versioning);
         ...
         var chain = SirixCompileChain.create()) {
      update(chain, ctx, "for $r in " + latestSource("[].rows[]")
          + " where $r.id eq 2 return delete json $r.value");
      ctx.applyUpdates();
    }
  }

  private Snapshot interpretedSnapshot(final VersioningType versioning, final int revision,
      final String[] sourcePath) throws Exception {
    try (var store = newStore(versioning);
         var ctx = SirixQueryContext.createWithJsonStore(store);
      ...
      return new Snapshot(
          run(chain, ctx, "count(for $r in " + source + " return $r.value)"),
          ...
          normalizeGroups(run(chain, ctx, "for $r in " + source
              + " let $g := $r.group group by $g return {\"group\":$g,\"count\":count($r)}")),
          normalizeGroups(run(chain, ctx, "for $r in " + source
              + " where $r.active let $g := $r.group group by $g "
              + "return {\"group\":$g,\"count\":count($r)}")),
          run(chain, ctx, "count(distinct-values(for $r in " + source + " return $r.group))"),
          normalizeGroups(run(chain, ctx, "for $r in " + source
              + " let $g := $r.group, $a := $r.active group by $g, $a "
              + "return {\"group\":$g,\"active\":$a,\"count\":count($r)}")));
    }
  }

  private Snapshot vectorizedSnapshot(final VersioningType versioning, final int revision,
      final String[] sourcePath) throws Exception {
    try (var store = newStore(versioning);
      ...
      try {
        return new Snapshot(
            serialize(executor.executeAggregate(null, sourcePath, "count", "value")),
  ...
  private static void update(final SirixCompileChain chain, final SirixQueryContext ctx,
      final String query) throws Exception {
    new Query(chain, query).evaluate(ctx);
  }

  private static String run(final SirixCompileChain chain, final SirixQueryContext ctx,
      final String query) throws Exception {
  ...
  private static void assertExpected(final Expected expected, final Snapshot actual,
    ...
    assertEquals(Long.toString(expected.predicateCount()), actual.predicateCount(),
        context + " predicate count");
    assertEquals(Long.toString(expected.predicateSum()), actual.predicateSum(),
        context + " predicate sum");
  ...
  private static String revisionSource(final int revision, final String[] sourcePath) {
    final StringBuilder source = new StringBuilder("jn:doc('")
        .append(DB)
        .append("','")
        .append(RES)
        .append("',")
