 ...
 *
 * <p>The projection kernels signal "not mine to serve" by throwing {@link IllegalStateException} —
 * a group column that is not a string dictionary is one such case — and the callers catch exactly
 ...
 *
 * <p>What makes it worth a test of this size is WHERE it hid. Below 64 row groups those drivers run
 ...
 * one — the shape of bug a suite of five-record fixtures cannot see. Found on a 3.48M-record corpus,
 * where {@code count(... group by $year)} against a projection carrying a numeric {@code year}
 * column failed outright where it used to answer in 1.5 ms.
 *
 * <p>The corpus is therefore deliberately past one row group per 1024 records × 64 — anything
 * smaller passes without the fix and proves nothing.
 */
final class ProjectionDeclineAtScaleTest {
  ...
  void setUp() throws Exception {
    ...
    for (int i = 0; i < N; i++) {
      ...
      sb.append("{\"year\":").append(1900 + i % 120).append(",\"title\":\"t").append(i % 997)
        .append("\"}");
    }
    sb.append(']');
