...
public class AuthorUUIDTest {
  ...
  public void test() throws IOException {
    ...
    try (
        final var database =
            Databases.openJsonDatabase(JsonTestHelper.PATHS.PATH1.getFile(), new User("johannes", johannesUUID));
    ...
    try (
        final var database =
            Databases.openJsonDatabase(JsonTestHelper.PATHS.PATH1.getFile(), new User("moshe", mosheUUID));
        final var manager = database.beginResourceSession("mydoc.jn");
        final var wtx = manager.beginNodeTrx()) {
      wtx.moveTo(2);
    ...
    try (
        final var database =
            Databases.openJsonDatabase(JsonTestHelper.PATHS.PATH1.getFile(), new User("carolin", carolinUUID));
        final var manager = database.beginResourceSession("mydoc.jn");
        final var wtx = manager.beginNodeTrx()) {
      wtx.moveTo(2);
      wtx.remove().commit();
