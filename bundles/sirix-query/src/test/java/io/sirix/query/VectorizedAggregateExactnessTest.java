 ...
 *       arbitrary precision; a long accumulator wraps after a few dozen 1e18-scale values and
 *       returned a silently wrong answer (ClickBench Q3 averages {@code UserID}). The expected
 *       values here are computed with {@link BigInteger}, not by the engine.</li>
 * </ol>
 */
public final class VectorizedAggregateExactnessTest {
  ...
  private static final long[] BIG_IDS = {
      1152908451924884486L, 1141889425024884481L, 1052908451924884482L, 1152908451924884483L,
      1052908451924884484L, 1152908451924884485L, 1052908451924884486L, 1152908451924884487L,
      1052908451924884488L, 1152908451924884489L, 1052908451924884490L, 1152908451924884491L,
      1052908451924884492L, 1152908451924884493L, 1052908451924884494L, 1152908451924884495L
  };
  ...
  public void sumAndAvgOverLargeIntegersStayExact() throws Exception {
    ...
    try (final BasicJsonDBStore store = newStore();
      ...
      final BigDecimal expectedAvg = new BigDecimal(expectedSum)
          .divide(BigDecimal.valueOf(BIG_IDS.length), MathContext.DECIMAL128).stripTrailingZeros();
      assertEquals(expectedAvg,
                   new BigDecimal(run(chain, ctx,
                                      "avg(for $r in jn:doc('agg-db','records.jn')[] return $r.id)"))
                       .stripTrailingZeros(),
  ...
  private static String document() {
    ...
    for (int i = 0; i < BIG_IDS.length; i++) {
      ...
      sb.append("{\"id\":").append(BIG_IDS[i])
        .append(",\"day\":\"").append(DATES[i % DATES.length]).append("\"}");
    }
