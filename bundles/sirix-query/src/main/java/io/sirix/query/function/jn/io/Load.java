...
@FunctionAnnotation(description = "Store the given fragments in a collection. "
		...
		+ "a new collection will be created. ", parameters = {"$coll", "$res", "$fragments", "$create-new"})
public final class Load extends AbstractFunction {
	...
	private static void add(final JsonDBCollection coll, final String resName, final Sequence resources,
			final Object options) {
		if (resources instanceof Atomic res) {
			try (final JsonReader reader = new JsonReader(
					new InputStreamReader(URIHandler.getInputStream(res.stringValue())))) {
			...
			try (final Iter iter = seq.iterate()) {
				...
				for (Item item; (item = iter.next()) != null;) {
					try (final JsonReader reader = new JsonReader(
							new InputStreamReader(URIHandler.getInputStream(((Str) item).stringValue())))) {
	...
	private JsonDBCollection create(final JsonDBStore store, final String collName, final String resName,
		...
		if (resources instanceof Atomic atomic) {
			try (final JsonReader reader = new JsonReader(
					new InputStreamReader(URIHandler.getInputStream(atomic.stringValue())))) {
			...
			try (final Iter iter = seq.iterate()) {
				...
				for (Item item; (item = iter.next()) != null;) {
					jsonReaders.add(new JsonReader(
							new InputStreamReader(URIHandler.getInputStream(((Str) item).stringValue()))));
				}
