 ...
 *
 * <p>
 * {@link StoreBoundExecutorCache} lifts the resource out of the query text. That works for
 ...
 *
 * <p>
 * The resource cannot be read off the query, and the {@link io.brackit.query.QueryContext} that
 ...
 *
 * <p>
 * This only decides WHICH executor is built and asked the translate-time questions. Whether that
 ...
 *
 * <p>
 * Only the {@code (database, resource, revision)} triple is kept, never the item: a thread-local
 * holding a bound item would retain its read transaction — and through it a revision's pages — for
 * as long as the thread lives.
 *
 * <p>
 * Per thread because compilation is: two threads compiling different queries against different
...
final class BoundDocumentHint {
   ...
   */
  private static final ThreadLocal<StoreBoundExecutorCache.DocumentSource> LAST_BOUND = new ThreadLocal<>();
   ...
   *
   * <p>
   * Deliberately does not clear the hint for a non-document binding. A query binding both a document
   * and a scalar parameter would otherwise lose the document to whichever bind came last, and the
   * order in which a caller binds its variables is not something this can require.
   */
  static void remember(final Sequence sequence) {
    final StoreBoundExecutorCache.DocumentSource source = StoreBoundExecutorCache.boundDocumentSource(sequence);
    if (source != null) {
