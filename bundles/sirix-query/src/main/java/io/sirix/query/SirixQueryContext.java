...
public final class SirixQueryContext implements QueryContext, AutoCloseable {
	 ...
	 *
	 * @param nodeStore
	 *            the database node storage to use
	 * @param jsonItemStore
	 *            the database json item storage to use
	...
	private void commitXmlTrx(List<UpdateOp> updateList) {
		...
		updateList.stream().map(UpdateOp::getTarget).map(mapDBNodeToWtx).flatMap(Optional::stream)
				.filter(trx -> trxIDs.add(trx.getId())).forEach(trx -> trx.commit(commitMessage, commitTimestamp));
	}
