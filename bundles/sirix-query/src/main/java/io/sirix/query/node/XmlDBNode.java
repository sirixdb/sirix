...
public final class XmlDBNode extends AbstractTemporalNode<XmlDBNode> implements StructuredDBItem<XmlNodeReadOnlyTrx> {
	 ...
	 *
	 * @param rtx
	 *            {@link XmlNodeReadOnlyTrx} for providing reading access to the
	 *            underlying node
	 * @param collection
	 *            {@link XmlDBCollection} reference
	 */
		...
		if (other instanceof XmlDBNode node) {
			...
			try {
				...
				if (node.getKind() != Kind.NAMESPACE && node.getKind() != Kind.ATTRIBUTE
						&& node.getParent().getImmutableNode().getNodeKey() == ((XmlDBNode) other.getParent())
								.getImmutableNode().getNodeKey()) {
					retVal = true;
				}
	...
	public boolean isDocumentOf(final Node<?> other) {
		...
		if (getKind() == Kind.DOCUMENT && other instanceof XmlDBNode node) {
			...
			if (rtx.getRevisionNumber() == this.rtx.getRevisionNumber() && rtx.getResourceSession().getResourceConfig()
					.getID() == this.rtx.getResourceSession().getResourceConfig().getID()) {
				retVal = true;
			}
		}
		return retVal;
	...
	private XmlDBNode append(final XmlNodeReadOnlyTrx rtx, final NodeSubtreeParser parser) {
		try {
			...
			parser.parse(new SubtreeBuilder(collection, (XmlNodeTrx) rtx, InsertPosition.AS_RIGHT_SIBLING,
	...
	private XmlDBNode insertBefore(final XmlNodeTrx wtx, final Node<?> node) {
		try {
			...
			final SubtreeBuilder builder = new SubtreeBuilder(collection, wtx, InsertPosition.AS_LEFT_SIBLING,
					Collections.emptyList());
			node.parse(builder);
			wtx.moveTo(builder.getStartNodeKey());
		} catch (final SirixException e) {
	...
	private XmlDBNode insertAfter(final XmlNodeTrx wtx, final Node<?> node) {
		try {
			...
			final SubtreeBuilder builder = new SubtreeBuilder(collection, wtx, InsertPosition.AS_RIGHT_SIBLING,
					Collections.emptyList());
			node.parse(builder);
			wtx.moveTo(builder.getStartNodeKey());
		} catch (final SirixException e) {
	...
	public XmlDBNode getPrevious() {
		moveRtx();
		final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new PreviousAxis<>(rtx.getResourceSession(),
				rtx);
		return moveTemporalAxis(axis);
	}
	 ...
	 * @throws SirixException
	 *             if Sirix fails to get the path class record
	 */
	public long getPCR() {
	 ...
	 *         {@link BasicXmlDBStore} configuration)
	 */
