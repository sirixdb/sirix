...
/**
 * <p>
 * Function for selecting a node in the revision it first existed. The parameter
 * is the context node. Supported signature is:
 * </p>
 * <ul>
...
public final class FirstExisting extends AbstractFunction {
	...
	public Sequence execute(final StaticContext sctx, final QueryContext ctx, final Sequence[] args) {
		...
		final RevisionReferencesNode indexNode = item.getTrx().getPageTrx().getRecord(item.getNodeKey(),
		...
		final var resourceManager = item.getTrx().getResourceSession();
		for (int revisionNumber = 1; revisionNumber <= resourceManager
				.getMostRecentRevisionNumber(); revisionNumber++) {
