...
public final class JsonDBArraySlice extends AbstractJsonDBArray<JsonDBArraySlice> {
	...
	public Stream<JsonDBArraySlice> getFuture(final boolean includeSelf) {
		moveRtx();
		final IncludeSelf include = includeSelf ? IncludeSelf.YES : IncludeSelf.NO;
		return new TemporalSirixJsonArraySliceStream(new FutureAxis<>(rtx.getResourceSession(), rtx, include),
	...
	public Sequence at(IntNumeric numericIndex) {
		...
		if (ii >= toIndex) {
			throw new QueryException(ErrorCode.ERR_INVALID_ARGUMENT_TYPE, "Invalid array index: %s",
					numericIndex.intValue());
		}
