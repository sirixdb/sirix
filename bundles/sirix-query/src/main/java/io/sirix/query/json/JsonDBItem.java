...
public interface JsonDBItem extends UpdatableJsonItem {
  ...
  /**
   * Gets or creates a write transaction, reverting to the source revision if needed.
   * This enables branching from historical revisions - when you open a document at
   * an old revision and make changes, the write transaction is reverted to that
   * revision's state before applying changes. The commit will create a new revision
   * while preserving all intermediate versions (SirixDB's append-only model).
   *
   ...
   * 
   * If the source document is from a historical revision, the write transaction is
   * reverted to that revision before applying the change, enabling branching from history.
   */
  @Override
