...
public final class JsonDBObject extends AbstractItem
		implements
			TemporalJsonDBItem<JsonDBObject>,
			Object,
			JsonDBItem,
	...
	public JsonDBObject getNext() {
		...
		final AbstractTemporalAxis<JsonNodeReadOnlyTrx, JsonNodeTrx> axis = new NextAxis<>(rtx.getResourceSession(),
				rtx);
	...
	public JsonDBObject getPrevious() {
		moveRtx();
		final AbstractTemporalAxis<JsonNodeReadOnlyTrx, JsonNodeTrx> axis = new PreviousAxis<>(rtx.getResourceSession(),
				rtx);
		return moveTemporalAxis(axis);
	}
	...
	public JsonDBObject getFirst() {
		moveRtx();
		final AbstractTemporalAxis<JsonNodeReadOnlyTrx, JsonNodeTrx> axis = new FirstAxis<>(rtx.getResourceSession(),
				rtx);
		return moveTemporalAxis(axis);
	}
	...
	public JsonDBObject getLast() {
		moveRtx();
		final AbstractTemporalAxis<JsonNodeReadOnlyTrx, JsonNodeTrx> axis = new LastAxis<>(rtx.getResourceSession(),
				rtx);
		return moveTemporalAxis(axis);
	}
	...
	public boolean booleanValue() {
		throw new QueryException(ErrorCode.ERR_ITEM_HAS_NO_TYPED_VALUE,
				"The boolean value of record items is undefined");
	}
		...
		if (value instanceof Atomic) {
			switch (value) {
				...
				case Numeric ignored1 -> {
					switch (value) {
						case Int anInt ->
							trx.insertObjectRecordAsLastChild(fieldName, new NumberValue(anInt.intValue()));
						case Int32 int32 ->
							trx.insertObjectRecordAsLastChild(fieldName, new NumberValue(int32.intValue()));
						case Int64 int64 ->
							trx.insertObjectRecordAsLastChild(fieldName, new NumberValue(int64.longValue()));
						case Flt flt -> trx.insertObjectRecordAsLastChild(fieldName, new NumberValue(flt.floatValue()));
						case Dbl dbl ->
							trx.insertObjectRecordAsLastChild(fieldName, new NumberValue(dbl.doubleValue()));
						case Dec dec ->
							trx.insertObjectRecordAsLastChild(fieldName, new NumberValue(dec.decimalValue()));
