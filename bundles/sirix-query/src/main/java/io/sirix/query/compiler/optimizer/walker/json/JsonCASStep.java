...
public final class JsonCASStep extends AbstractJsonPathWalker {
					...
					if (indexAstNode.getType() == XQ.SequenceExpr && indexAstNode.getChildCount() == 0) {
						...
						if (derefAstNode.getType() == XQ.FunctionCall) {
							...
							adaptPathNamesToArrayIndexesWithNewArrayIndex(pathSegmentName,
									predicatePathSegmentsToArrayIndexes, Integer.MIN_VALUE);
	...
	protected AST visit(AST astNode) {
		...
		if (astNode.getChildCount() != 2
				|| (astNode.getChild(0).getType() != XQ.ArrayAccess && astNode.getChild(0).getType() != XQ.DerefExpr
		...
		if (predicateChildAstNode.getType() == XQ.AndExpr) {
			...
			if (!"ValueCompGT".equals(comparator) && !"GeneralCompGT".equals(comparator)
					&& !"ValueCompGE".equals(comparator) && !"GeneralCompGE".equals(comparator)) {
				return null;
			}

			final var node = processPredicateChildAstNode(astNode, leftChild, predicateChildAstNode.getChild(1), false,
					false);
