...
public final class XmlDBCollection extends AbstractNodeCollection<AbstractTemporalNode<XmlDBNode>>
		implements
			TemporalNodeCollection<AbstractTemporalNode<XmlDBNode>>,
			AutoCloseable {
	...
	private XmlDBNode getDocumentInternal(final String resName, final Instant pointInTime) {
		return instantDocumentDataToXmlDBNodes.computeIfAbsent(new InstantDocumentData(resName, pointInTime),
				(unused) -> {
	...
	private XmlDBNode createResource(NodeSubtreeParser parser, final String resourceName, final String commitMessage,
			final Instant commitTimestamp) {
		database.createResource(ResourceConfiguration.newBuilder(resourceName).useDeweyIDs(true)
				.useTextCompression(true).buildPathSummary(true).customCommitTimestamps(commitTimestamp != null)
		...
		final NodeSubtreeHandler handler = new SubtreeBuilder(this, wtx, InsertPosition.AS_FIRST_CHILD,
		...
		instantDocumentDataToXmlDBNodes.put(new InstantDocumentData(resourceName, wtx.getRevisionTimestamp()),
				xmlDBNode);
		return xmlDBNode;
	}
