	...
	private SirixJsonLazySequence getLazySequence(String recordFieldAsString, JsonDBItem jsonDBItem,
		...
		if (resourceManager.getResourceConfig().withPathSummary && rtx.hasChildren()) {
					...
					if (matches.cardinality() == 1) {
						...
						if (matchLevel == startLevel + 1) {
							if (reader.getPathNode().getKind() == NodeKind.ARRAY) {
								return getLazySequence(
										new SirixJsonStream(new ChildAxis(rtx), jsonDBItem.getCollection()));
							}
							if (nodeKind == NodeKind.ARRAY) {
								var axis = new NestedAxis(
										new FilterAxis<>(new NestedAxis(new ChildAxis(rtx), new ChildAxis(rtx)),
												new ObjectKeyFilter(rtx), new JsonNameFilter(rtx, recordFieldAsString)),
										new ChildAxis(rtx));
						...
						return getLazySequence(
								new SirixJsonStream(buildQuery(pathSegments, resourceManager, revisionNumber, nodeKey),
					...
					if (onSameLevel) {
						...
						for (int j = level; (j > startLevel && nodeKind == NodeKind.OBJECT)
							...
							for (i = matches.nextSetBit(0); i >= 0; i = matches.nextSetBit(i + 1)) {
								...
								pathSegmentBuilder
										.add(new PathSegment(reader.getName(), reader.getPathKind() == NodeKind.ARRAY));
							}
							...
							if (pathSegment.isArray) {
								...
								if (j == startLevel && nodeKind == NodeKind.ARRAY) {
									if (sameName) {
										axis = new FilterAxis<>(
												new NestedAxis(new ChildAxis(newRtx), new ChildAxis(newRtx)),
						...
						for (i = matches.nextSetBit(0); i >= 0; i = matches.nextSetBit(i + 1)) {
							...
							if (matchLevel == level + 1) {
								...
								if (reader.getPathNode().getKind() == NodeKind.ARRAY) {
									...
									if (nodeKind == NodeKind.ARRAY) {
										filterAxis = new FilterAxis<>(
												new NestedAxis(new ChildAxis(newConcurrentRtx),
										...
										axisQueue.addLast(
												new NestedAxis(new ConcurrentAxis<>(newConcurrentRtx1, filterAxis),
			...
			final var filterAxis = new NestedAxis(
					new FilterAxis<>(new DescendantAxis(rtx), new JsonNameFilter(rtx, recordFieldAsString)),
					new ChildAxis(rtx));
		...
		for (int i = 0, size = pathSegments.size(); i < size; i++) {
			...
			if (!pathSegment.isArray) {
				if (lastPathSegmentIsArray) {
					...
					axis = new NestedAxis(axis, new FilterAxis<>(new ChildAxis(concurrentRtx),
							new ObjectKeyFilter(concurrentRtx), new JsonNameFilter(concurrentRtx, pathSegment.name)));
				}
