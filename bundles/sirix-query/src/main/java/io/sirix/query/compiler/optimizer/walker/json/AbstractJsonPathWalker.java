...
abstract class AbstractJsonPathWalker extends ScopeWalker {
	...
	protected AST replaceAstIfIndexApplicable(AST astNode, AST predicateNode, Type type) {
		...
		try (final var jsonCollection = jsonDBStore.lookup(revisionData.databaseName());
			...
			// path node keys of all paths, which have the right most field of the query in
			// its path
			var queryPathSegment = pathSegmentNamesToArrayIndexes.getLast();
			var pathNodeKeys = findFurthestFromRootPathNodes(astNode,
					!queryPathSegment.arrayIndexes().isEmpty() ? "__array__" : queryPathSegment.pathSegmentName(),
					pathSummary, !queryPathSegment.arrayIndexes().isEmpty() ? NodeKind.ARRAY : NodeKind.OBJECT_KEY);

			var pathNodeKeysToRemove = pathNodeKeys.stream().filter(pathNodeKey -> Paths
					.isPathNodeNotAQueryResult(pathSegmentNamesToArrayIndexes, pathSummary, pathNodeKey)).toList();
			...
			boolean notFound = findIndexDefsForPathNodeKeys(predicateNode, type, predicateSegmentNames, revisionData,
					resMgr, pathSummary, pathNodeKeys, foundIndexDefsToPaths, foundIndexDefsToPredicateLevels);
	...
	private Deque<String> toPredicateSegmentNames(final Deque<QueryPathSegment> predicateSegmentNamesToArrayIndexes) {
		return predicateSegmentNamesToArrayIndexes.stream().map(QueryPathSegment::pathSegmentName)
	...
	protected PathData traversePath(final AST node, final AST predicateNode) {
		...
		if (node.getType() == XQ.DerefExpr) {
			// otherwise the pathSegmentName has been added in the getPredicatePathStep
			// method already (only if context item _plus_ array index access expr)
			...
			if (node.getChildCount() == 2) {
				...
				final Optional<AST> derefNode = processLastArrayAccess(pathSegmentNamesToArrayIndexes, node,
						arrayAstNode, false);
		...
		return newNode
				.map(unwrappedNode -> new PathData(pathSegmentNamesToArrayIndexes,
						predicatePathSegmentNamesToArrayIndexes, unwrappedNode, predicateLeafNode.orElse(null)))
				.orElse(null);
	}
	...
	private boolean findIndexDefsForPathNodeKeys(AST predicateNode, Type type, Deque<String> predicateSegmentNames,
			RevisionData revisionData, JsonResourceSession resMgr, PathSummaryReader pathSummary,
	...
	Optional<AST> getPathStep(AST node, Deque<QueryPathSegment> pathSegmentNamesToArrayIndexes) {
		for (int i = 0, length = node.getChildCount(); i < length; i++) {
			...
			if (step.getType() == XQ.VariableRef) {
				...
				return optionalLetBindNode
						.map(returnFunctionCallOrIndexExprNodeIfPresent(pathSegmentNamesToArrayIndexes));
			}
	...
	private AST processForBind(Deque<QueryPathSegment> pathSegmentNamesToArrayIndexes, AST node) {
		...
		if (stepNode.getType() == XQ.DerefExpr) {
			...
			final Deque<QueryPathSegment> currentPathSegmentNamesToArrayIndexes = (Deque<QueryPathSegment>) stepNode
					.getProperty("pathSegmentNamesToArrayIndexes");
	...
	protected String getPathNameFromContextItem(AST possiblyDerefAstNode) {
		...
		while (possiblyDerefAstNode.getParent() != null
				&& possiblyDerefAstNode.getParent().getType() != XQ.FilterExpr) {
