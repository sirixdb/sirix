...
/**
 * <p>
 * Function for getting the item in all revisions in which it has been changed
 * (order ascending). Supported signature is:
 * </p>
 * <ul>
			...
			while (revision > 0) {
				...
				if (rtxInRevision.moveTo(item.getNodeKey())) {
					if (rtxInRevision instanceof XmlNodeReadOnlyTrx) {
						...
						final var xmlDBNode = new XmlDBNode((XmlNodeReadOnlyTrx) rtxInRevision,
								((XmlDBNode) item).getCollection());
						sequences.addFirst(xmlDBNode);
					} else if (rtxInRevision instanceof JsonNodeReadOnlyTrx trx) {
						assert item instanceof JsonDBItem;
						final var jsonItem = new JsonItemFactory().getSequence(trx,
								((JsonDBItem) item).getCollection());
			...
			for (final int revision : revisions) {
				...
				if (rtxInRevision.moveTo(item.getNodeKey())) {
					if (rtxInRevision instanceof XmlNodeReadOnlyTrx) {
						assert item instanceof XmlDBNode;
						sequences.add(
								new XmlDBNode((XmlNodeReadOnlyTrx) rtxInRevision, ((XmlDBNode) item).getCollection()));
