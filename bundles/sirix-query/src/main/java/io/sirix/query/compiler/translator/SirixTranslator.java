...
import io.sirix.axis.filter.xml.*;
...
public final class SirixTranslator extends TopDownTranslator {
	...
	/**
	 * Number of children (needed as a threshold to lookup in path summary if a path
	 * exists at all).
	 */
	public static final int CHILD_THRESHOLD = Cfg.asInt("org.sirix.xquery.optimize.child.threshold", 100_000);

	/**
	 * Number of descendants (needed as a threshold to lookup in path summary if a
	 * path exists at all).
	 */
	...
	private static final class Last extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			...
			final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new LastAxis<>(rtx.getResourceSession(),
					rtx);
			return new TemporalSirixNodeStream(SirixTranslator.getTemporalAxis(test, rtx, axis),
	...
	private static final class First extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new FirstAxis<>(rtx.getResourceSession(),
					rtx);
			return new TemporalSirixNodeStream(SirixTranslator.getTemporalAxis(test, rtx, axis),
					dbNode.getCollection());
		}
	...
	private static final class Next extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new NextAxis<>(rtx.getResourceSession(),
					rtx);
			return new TemporalSirixNodeStream(SirixTranslator.getTemporalAxis(test, rtx, axis),
					dbNode.getCollection());
		}
	...
	private static final class Previous extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new PreviousAxis<>(
					rtx.getResourceSession(), rtx);
			return new TemporalSirixNodeStream(SirixTranslator.getTemporalAxis(test, rtx, axis),
					dbNode.getCollection());
		}
	...
	private static final class AllTime extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new AllTimeAxis<>(
					rtx.getResourceSession(), rtx);
			return new TemporalSirixNodeStream(SirixTranslator.getTemporalAxis(test, rtx, axis),
					dbNode.getCollection());
		}
	...
	private static final class Past extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new PastAxis<>(rtx.getResourceSession(),
					rtx, mSelf);
			return new TemporalSirixNodeStream(SirixTranslator.getTemporalAxis(test, rtx, axis),
					dbNode.getCollection());
		}
	...
	private static final class Future extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			final AbstractTemporalAxis<XmlNodeReadOnlyTrx, XmlNodeTrx> axis = new FutureAxis<>(rtx.getResourceSession(),
					rtx, includeSelf);
			return new TemporalSirixNodeStream(SirixTranslator.getTemporalAxis(test, rtx, axis),
					dbNode.getCollection());
		}
	...
	private static final class Preceding extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			return new SirixNodeStream(SirixTranslator.getAxis(test, rtx, new PrecedingAxis(rtx)),
					dbNode.getCollection());
		}
	...
	private static final class Following extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			final XmlDBNode dbNode = (XmlDBNode) node;
			final XmlNodeReadOnlyTrx rtx = dbNode.getTrx();
			return new SirixNodeStream(SirixTranslator.getAxis(test, rtx, new FollowingAxis(rtx)),
					dbNode.getCollection());
		}
	...
	private static final class Attribute extends Accessor {
		...
		public Stream<? extends Node<?>> performStep(final Node<?> node, final NodeType test) {
			...
			return new SirixNodeStream(
					new FilterAxis<>(new AttributeAxis(rtx), new XmlNameFilter(rtx, test.getQName())),
					dbNode.getCollection());
		}
	...
	private static final class DescOrSelf extends Accessor {
		...
		private static io.sirix.api.Axis buildQuery(final XmlNodeReadOnlyTrx rtx, final Deque<QNm> names) {
			io.sirix.api.Axis axis = new FilterAxis<>(new ChildAxis(rtx), new ElementFilter(rtx),
