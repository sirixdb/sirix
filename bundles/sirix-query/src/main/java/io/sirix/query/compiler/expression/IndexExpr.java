...
public final class IndexExpr implements Expr {
	...
	public Sequence evaluate(QueryContext ctx, Tuple tuple) throws QueryException {
		...
		final JsonNodeReadOnlyTrx rtx = revision == -1
		...
		final var pathSegmentNamesToArrayIndexes = (Deque<QueryPathSegment>) properties
				.get("pathSegmentNamesToArrayIndexes");
			...
			switch (indexType) {
				case PATH -> {
					final Iterator<NodeReferences> nodeReferencesIterator = indexController.openPathIndex(
							rtx.getPageTrx(), entrySet.getKey(), indexController.createPathFilter(pathStrings, rtx));

					checkIfIndexNodeIsApplicable(manager, rtx, pathSegmentNamesToArrayIndexes, nodeReferencesIterator,
							nodeKeys, false);
				}
				case CAS -> {
					...
					if (atomicUpperBound != null && comparisonUpperBound != null) {
						...
						if ((searchMode != SearchMode.GREATER && searchMode != SearchMode.GREATER_OR_EQUAL)
								|| (searchModeUpperBound != SearchMode.LOWER
						...
						final var casFilter = new CASFilterRange(new HashSet<>(entrySet.getValue()), atomic,
								atomicUpperBound, searchMode == SearchMode.GREATER_OR_EQUAL,
								searchModeUpperBound == SearchMode.LOWER_OR_EQUAL, new JsonPCRCollector(rtx));

						final Iterator<NodeReferences> nodeReferencesIterator = indexController
								.openCASIndex(rtx.getPageTrx(), entrySet.getKey(), casFilter);

						checkIfIndexNodeIsApplicable(manager, rtx, pathSegmentNamesToArrayIndexes,
								nodeReferencesIterator, nodeKeys, false);

					} else {

						final var casFilter = new CASFilter(new HashSet<>(entrySet.getValue()), atomic, searchMode,
				...
				case NAME -> {
					final Iterator<NodeReferences> nodeReferencesIterator = indexController
							.openNameIndex(rtx.getPageTrx(), entrySet.getKey(), new NameFilter(
									Set.of(entrySet.getValue().get(entrySet.getValue().size() - 1).tail()), Set.of()));
		...
		switch (indexType) {
			...
			case CAS -> indexDefsToPaths.keySet().forEach(indexDef -> {
				...
				nodeKeysOfIndex.forEach(nodeKey -> {
					...
					// Then we can simply clip the DeweyID with the given path level and get the
	...
	private void checkIfIndexNodeIsApplicable(JsonResourceSession manager, JsonNodeReadOnlyTrx rtx,
		...
		try (final var pathSummary = revision == -1 ? manager.openPathSummary() : manager.openPathSummary(revision)) {
			nodeReferencesIterator.forEachRemaining(currentNodeReferences -> {
				...
				// if array numberOfArrayIndexes are given (only some might be specified we have
				// to drop false positive nodes
				if (numberOfArrayIndexes != 0 || checkPathBecauseOfFieldNameChecks) {
					...
					while (nodeKeyIter.hasNext()) {
						...
						pathSegmentNamesToArrayIndexes.forEach(pathSegmentNameToArrayIndex -> {
							...
							currentPathSegmentNamesToArrayIndexes.addLast(new QueryPathSegment(
									pathSegmentNameToArrayIndex.pathSegmentName(), currentIndexes));
						});
						...
						if (checkPathBecauseOfFieldNameChecks) {
							if (Paths.isPathNodeNotAQueryResult(pathSegmentNamesToArrayIndexes, pathSummary,
									pathNodeKey)) {
						...
						outer : for (int i = steps.size() - 1; i >= 0; i--) {
							...
							if (step.getAxis() == Path.Axis.CHILD_ARRAY) {
								...
								if (indexes == null) {
									...
									// at least some array numberOfArrayIndexes are given for the specific object
									// key node
									int y = 0;
									for (int m = 0, length = currentIndex - j - indexes.size(); m < length; m++) {
										// for instance =>foo[[0]]=>bar in a path /foo/[]/[]/[]/bar meaning at least one
	...
	private long getNumberOfArrayIndexes(Deque<QueryPathSegment> pathSegmentNamesToArrayIndexes) {
		return pathSegmentNamesToArrayIndexes.stream().map(QueryPathSegment::arrayIndexes)
				.filter(arrayIndexes -> !arrayIndexes.isEmpty()).count();
	}
