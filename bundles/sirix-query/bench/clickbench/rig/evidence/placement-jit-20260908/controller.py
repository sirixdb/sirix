...
parser.add_argument('--cohort', choices=('full','fast33'), required=True)
...
if args.cohort=='fast33':
    cmd[cmd.index('--tries')+1]='33'
    cmd.extend(['--queries','2,3,6,21,42'])
cmd.insert(1, '-XX:StartFlightRecording=filename='+str(out/'recording.jfr')+',settings='+str(WORK/'jvm50-01/diagnostic.jfc')+',dumponexit=true')
(out/'protocol.json').write_text(json.dumps({'scope':'diagnostic-only JFR plus main-thread CPU placement sampling; no score/variance claim','cohort':args.cohort,'power_uw':50000000,'source':'unchanged aa4d81d54 compiled runtime','perf':'cpu-clock:u 999Hz main thread only; clockid mono; no affinity change'},indent=2)+'\n')
...
with RigLease(timeout=0) as lease:
        ...
        try:
            ...
            with (out/'telemetry.jsonl').open('x') as stream:
                while not done.is_set():
                    ...
                    stream.write(json.dumps(row)+'\n');stream.flush()
                    if profiler is not None and profiler.poll() not in (None, 0) and proc.poll() is None:
                        raise RuntimeError('perf ended while the query JVM was still alive')
    ...
    try:
        with (out/'suite100m.log').open('x') as log, (out/'query-boundaries.jsonl').open('x') as boundaries:
            ...
            main_tid=None
            while main_tid is None:
                tids=[]
                for task in Path(f'/proc/{proc.pid}/task').glob('*'):
                    try:
                        if task.name!=str(proc.pid) and (task/'comm').read_text().strip()=='java': tids.append(int(task.name))
                    except FileNotFoundError: pass
                if len(tids)==1: main_tid=tids[0]
                elif len(tids)>1: raise RuntimeError(f'ambiguous Java main native thread: {tids}')
                elif proc.poll() is not None or time.monotonic()>deadline: raise RuntimeError('Java main native thread not found before sampling')
                else: time.sleep(.01)
            perf_argv=['perf','record','--quiet','--sample-cpu','--clockid','mono','-e','cpu-clock:u','-F','999','-t',str(main_tid),'-o',str(out/'main-cpu.data')]
            (out/'perf-command.json').write_text(json.dumps(dict(argv=perf_argv,main_tid=main_tid,epoch=time.time(),monotonic=time.monotonic()),indent=2)+'\n')
            perf_log=(out/'perf.log').open('x')
            profiler=subprocess.Popen(perf_argv,stdout=perf_log,stderr=subprocess.STDOUT)
            for line in proc.stdout:
                log.write(line);log.flush()
                if line.startswith('# q') and ' try ' in line:
                    row={'received_epoch':time.time(),'received_monotonic':time.monotonic(),'line':line.strip()}
        ...
        if profiler is not None:
            ...
            perf_code=profiler.wait()
            if perf_code not in (0,130,-2): issues.append(f'perf exit {perf_code}')
        if perf_log is not None: perf_log.close()
    after=snap()
