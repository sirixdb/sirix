...
def main():
    ...
    reference = None
    profile_reference = answers(Path(__file__).resolve().parent / 'baseline-answers')
    current_output = None
    launches = 0

    def protocol(args):
        if args.pairs != 12 or args.effect_ln != 1.5 or args.seed != 0:
            raise ValueError('this experiment prespecifies 12 pairs, effect-ln 1.5, seed 0')
        expected_diag = ['-Dsirix.projection.groupPasses.planDiag=true']
        if args.baseline_jvm_arg != expected_diag or args.candidate_jvm_arg != expected_diag:
            raise ValueError('both arms must carry the same single group-plan diagnostic argument')
        ...
        value['answer_validation'] = dict(
            ...
            profiling_jvms_already_used=PROFILING_JVMS_ALREADY_USED)
        return value
    ...
    def run_leg(runtime, database, output, protocol, lease):
        ...
        if reference is None:
            ...
            if observed != profile_reference:
                mismatches = [name for name, data in observed.items() if data != profile_reference[name]]
                raise RuntimeError(f'baseline answers differ from the pre-change full-suite profile: {mismatches}')
            reference = observed
