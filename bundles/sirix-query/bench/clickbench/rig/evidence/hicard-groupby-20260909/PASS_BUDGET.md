...
A completed scan memoes its exact group count and pass count, and a later execution replays that pass count while each pass stays within twice the current budget. That multiple was calibrated for the shared quarter share, where twice the budget is at most half the headroom. Under the bounded three-quarter share it is unsafe: with q32's memo of (100,007,737 groups, 2 passes), an execution whose headroom has fallen to 4 GiB reads a budget of 28,760,941 groups, and replaying two passes sets the pass budget to 52,546,486 groups — the 50,003,869-group largest pass plus the skew margin — charging 5.48 GiB of tables (at 112 B) into that 4 GiB. Bounded plans therefore replay a memoed pass count only while each pass fits the current budget and otherwise take the passes the count implies: four at 4 GiB, at a pass budget of 28,760,941 groups charging 3.00 GiB. The shared share keeps its calibrated multiple. The paired legs bracketed the profile's 7,607 MiB rather than running at it: across all 36 q32 candidate plan records (`candidate_pass_records` in `study-summary.json`, summarising the `[groupPlan]` lines archived in `comparison.tar.gz`) the bounded budget ranges from 51,370,492 groups to the 2^26 cap, and across the 24 hot records from 51,370,492 to 60,655,244 groups; at 112 B the minimum is about 7.14 GiB (7,316 MiB) of effective headroom. Even that minimum exceeds the largest two-pass share — 50,003,869 groups at the profile's 100,007,737 estimate, 49,998,747 at the 99,997,493 groups every candidate leg counted — so at every recorded budget, as at the profile's, two passes fit the current budget outright and both policies plan the same two passes for q32: replaying all 720 bounded plan records in the candidate legs changes no pass count, and this review fix changes no measured leg. (A replayed pass budget is then widened to the largest pass plus the skew margin: 52,541,103 groups at the measured 99,997,493, which is why two hot records show a pass budget above their budget; the 52,546,486 above is the same figure at the 100,007,737-group estimate the fixture uses.) `GroupPassesBudgetRefreshTest` fixes the sequence as planning arithmetic without a 100M run.
