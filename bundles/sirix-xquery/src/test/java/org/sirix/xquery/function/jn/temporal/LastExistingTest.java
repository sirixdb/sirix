...
public final class LastExistingTest {
  ...
  public void test_whenRevisionsAndNodeExists_getRevision() throws IOException {
    try (final var store = BasicJsonDBStore.newBuilder().location(SIRIX_DB_PATH).build();
      ...
      final var allTimesQuery = "sdb:revision(jn:last-existing(sdb:select-item(jn:doc('mycol.jn','mydoc.jn'), 26)))";
  ...
  public void test_whenNodeDoesNotExistInLastIndexedRevision_getRevision() throws IOException {
    try (final var store = BasicJsonDBStore.newBuilder().location(SIRIX_DB_PATH).build();
      ...
      final var allTimesQuery = "sdb:revision(jn:last-existing(sdb:select-item(jn:doc('mycol.jn','mydoc.jn',2), 11)))";
      final var allTimesSeq = new XQuery(chain, allTimesQuery).execute(ctx);
