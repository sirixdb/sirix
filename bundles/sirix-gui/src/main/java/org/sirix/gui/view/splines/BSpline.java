...
public class BSpline implements Spline {
	...
	@Override
	public void draw(final AbstractSunburstGUI pGUI, final PGraphics pGraphic,
			final List<PVector> pPath) {
		...
		for (; i < path.size() - 3; i += 3) {
			pGraphic.bezierVertex(path.get(i).x, path.get(i).y, path.get(i + 1).x,
					path.get(i + 1).y, path.get(i + 2).x, path.get(i + 2).y);
		}

		final PVector end = new PVector(pPath.get(pPath.size() - 1).x,
				pPath.get(pPath.size() - 1).y);
		if (i - 1 > 0 && i < path.size()) {
			pGraphic.bezierVertex(path.get(i - 1).x, path.get(i - 1).y,
					path.get(i).x, path.get(i).y, end.x, end.y);
			pGUI.drawArrow(pGraphic, path.get(i).x, path.get(i).y, end.x, end.y,
					EDrawLine.NO);
		} else {
