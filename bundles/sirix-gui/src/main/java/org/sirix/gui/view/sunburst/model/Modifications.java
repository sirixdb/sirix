...
public final class Modifications extends RecursiveTask<Modification> implements
	...
	private static final LogWrapper LOGWRAPPER = new LogWrapper(
			LoggerFactory.getLogger(Modifications.class));
	 ...
	 */
	public static Modifications getInstance(final int paramIndex,
			final Map<Integer, DiffTuple> paramDiffs) throws SirixException {
	 ...
	 * @param param
	 */
	private Modifications(final int paramIndex,
			final Map<Integer, DiffTuple> paramDiffs) {
		mIndex = paramIndex;
		mDiffs = paramDiffs;
	}
	...
	private int incrDiffCounter(final int paramIndex, final int paramDiffCounts) {
		...
		if (paramIndex < mDiffs.size()
				&& mDiffs.get(paramIndex).getDiff() != DiffType.SAME
	...
	public Modification countDiffs() throws SirixException {
		...
		final int rootDepth = (diff == DiffType.DELETED
				|| diff == DiffType.MOVEDFROM || diff == DiffType.REPLACEDOLD) ? diffCont
				.getDepth().getOldDepth() : diffCont.getDepth().getNewDepth();
		...
		if (diffCounts == 1 && index < mDiffs.size()) {
			...
			final int depth = (cont.getDiff() == DiffType.DELETED
					|| cont.getDiff() == DiffType.MOVEDFROM || cont.getDiff() == DiffType.REPLACEDOLD) ? cont
					.getDepth().getOldDepth() : cont.getDepth().getNewDepth();
		...
		while (!done && index < mDiffs.size()) {
			...
			final int depth = (currDiff == DiffType.DELETED
					|| currDiff == DiffType.MOVEDFROM || currDiff == DiffType.REPLACEDOLD) ? currDepth
					.getOldDepth() : currDepth.getNewDepth();
			if (depth <= rootDepth) {
				done = true;
			}
			if (!done) {
		...
		return new Modification(TraverseModel.FACTOR * diffCounts,
				descendantCounts, subtract);
	}
