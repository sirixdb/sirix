...
public final class QueryUsage {
	...
	private static void loadDocumentAndQueryTemporal() throws QueryException, IOException, SirixException {
		...
		// final CompileChain compileChain =
		// SirixCompileChain.createWithNodeStore(store);
		// final QueryContext ctx1 = SirixQueryContext.createWithNodeStore(store);
		// final String query1
		...
		// System.out.println("Insert into loaded document:");
		// final String xq2 =
		// "insert nodes <a><b/>test<c/>55<d>22</d></a> into sdb:doc('mydocs.col',
		// 'resource1', (),
		// fn:boolean(1))/log";
		// System.out.println(xq2);
		...
		try (final BasicXmlDBStore store = BasicXmlDBStore.newBuilder().build()) {
			...
			final Query q = new Query(SirixCompileChain.createWithNodeStore(store),
					"let $doc := xml:doc('mydocs.col', 'resource1', (), fn:boolean(1)) "
							+ "let $stats := xml:create-path-index($doc, '//*') "
							+ "return <rev>{sdb:commit($doc)}</rev>");
			q.serialize(ctx3, System.out);
			System.out.println();
		...
		try (final BasicXmlDBStore store = BasicXmlDBStore.newBuilder().build()) {
			final QueryContext ctx3 = SirixQueryContext.createWithNodeStoreAndCommitStrategy(store,
		...
		try (final BasicXmlDBStore store = BasicXmlDBStore.newBuilder().build()) {
			...
			final QueryContext ctx3 = SirixQueryContext.createWithNodeStore(store);
			final String query = "let $doc := xml:doc('mydocs.col', 'resource1') return xml:scan-cas-index($doc, sdb:find-cas-index($doc, 'xs:string', '//@*'), 'bar', true(), '==', ())";
		...
		try (final BasicXmlDBStore store = BasicXmlDBStore.newBuilder().build()) {
			...
			final QueryContext ctx3 = SirixQueryContext.createWithNodeStore(store);
			final String query = "let $doc := xml:doc('mydocs.col', 'resource1') return xml:scan-cas-index-range($doc, sdb:find-cas-index($doc, 'xs:integer', '//*'), 10, 100, true(), true(), ())";
			final Sequence seq = new Query(SirixCompileChain.createWithNodeStore(store), query).execute(ctx3);
			// final Iter iter = seq.iterate();
			// for (Item item = iter.next(); item != null; item = iter.next()) {
			// System.out.println(item);
		...
		try (final BasicXmlDBStore store = BasicXmlDBStore.newBuilder().build()) {
			...
			final QueryContext ctx3 = SirixQueryContext.createWithNodeStore(store);
			final XmlDBNode node = (XmlDBNode) new Query(SirixCompileChain.createWithNodeStore(store),
					"doc('mydocs.col')").execute(ctx3);
			final Optional<IndexDef> index = node.getTrx().getResourceSession()
