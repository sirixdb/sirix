...
public class SerializeVersionedXmlResource {
	...
	private static void serializeRevisionOneAndTwo(final XmlResourceSession manager) {
		...
		final var serializerForRevisionOneAndTwo = XmlSerializer.newBuilder(manager, outputStream, 1, 2).emitIDs()
				.prettyPrint().serializeTimestamp(true).build();
	...
	private static void serializeMostRecentRevision(final XmlResourceSession manager) {
		final var outputStream = new ByteArrayOutputStream();
		final var serializerForMostRecentRevision = XmlSerializer.newBuilder(manager, outputStream).emitIDs()
				.prettyPrint().serializeTimestamp(true).build();
	...
	private static void serializeAllRevisions(final XmlResourceSession manager) {
		final var outputStream = new ByteArrayOutputStream();
		final var serializerForAllRevisions = XmlSerializer.newBuilder(manager, outputStream, -1).emitIDs()
				.prettyPrint().serializeTimestamp(true).build();
