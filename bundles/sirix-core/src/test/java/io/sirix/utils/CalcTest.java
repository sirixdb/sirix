...
public class CalcTest {
  ...
  public void compareUAsPrefix() {
    ...
    assertEquals("Null, which should be maximum value, is smaller that non null.", -1,
        Calc.compareUAsPrefix(value1, null));
    assertEquals("Null, which should be maximum value, is smaller that non null.", 1,
        Calc.compareUAsPrefix(null, value1));
    ...
    assertEquals("Comparison where the first value is larger than the second gave error.", 1,
        Calc.compareUAsPrefix(value1, value2));
    byte[] value3 = new byte[] {2, 2, 2, 2, 2};
    assertEquals("Comparison where one is longer than the other gave the incorrect result", 1,
        Calc.compareUAsPrefix(value3, value1));
  }
  ...
  public void compareAsPrefix() {
    byte[] value1 = new byte[] {2, 2, 2, 2};
    assertEquals("Null, which should be maximum value, is smaller that non null.", -1,
        Calc.compareAsPrefix(value1, null));
    assertEquals("Null, which should be maximum value, is smaller that non null.", 1,
        Calc.compareAsPrefix(null, value1));
    assertEquals("Both are the largest possible value and thus equal.", 0, Calc.compareAsPrefix(null, null));
    byte[] value2 = new byte[] {1, 1, 1, 1};
    assertEquals("Comparison where the first value is larger than the second gave error.", 1,
        Calc.compareAsPrefix(value1, value2));
    byte[] value3 = new byte[] {2, 2, 2, 2, 2};
    assertEquals("Comparison where one is longer than the other gave the incorrect result", 1,
        Calc.compareAsPrefix(value3, value1));
  }
