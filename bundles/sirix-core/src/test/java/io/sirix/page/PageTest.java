...
public class PageTest {
	...
	public void testByteRepresentation(final Page[] handlers) {
		for (final Page handler : handlers) {
			...
			PageKind.getKind(handler.getClass()).serializePage(pageReadTrx.getResourceSession().getResourceConfig(),
					data, handler, SerializationType.DATA);
	...
	public Object[][] instantiatePages() throws SirixIOException {
		...
		final KeyValueLeafPage nodePage = new KeyValueLeafPage(XmlTestHelper.random.nextInt(Integer.MAX_VALUE),
				IndexType.DOCUMENT, pageReadTrx.getResourceSession().getResourceConfig(),
	...
	private PageTrx createPageTrxMock() {
		...
		when(pageTrx.createRecord(any(HashCountEntryNode.class), eq(IndexType.NAME), eq(0)))
				.thenReturn(hashCountEntryNode);
