...
/**
 * Test to verify DeweyID encoding properties for the formal proof document. This test generates
 * test vectors and verifies order preservation.
 */
class DeweyIDEncodingVerificationTest {

  @Test
  void generateTestVectors() {
    String[] testCases = {"1", "1.0", "1.1", "1.3", "1.17", "1.127", "1.128", "1.3.5", "1.3.5.7", "1.16511", "1.16512"};
    ...
    for (String tc : testCases) {
      ...
      String hexStr = bytes.length == 0
          ? "(empty)"
          : hex.formatHex(bytes);
      System.out.printf("| %s | %s | %s | %d |%n", tc, Arrays.toString(id.getDivisionValues()), hexStr, bytes.length);
    }
  }
  ...
  void verifyOrderPreservation() {
    ...
    for (int i = 0; i < ordered.length - 1; i++) {
      ...
      System.out.printf("%s < %s: obj=%d, bytes=%d %s%n", ordered[i], ordered[i + 1], objCmp, byteCmp, status);

      assertTrue(orderPreserved, "Order not preserved for " + ordered[i] + " < " + ordered[i + 1]);
    }
  }
  ...
  void verifyTierBoundaries() {
    ...
    for (int boundary : tierBoundaries) {
      ...
      System.out.printf("Boundary %d: len=%d, hex=%s%n", boundary, bytesAt.length, hex.formatHex(bytesAt));
      System.out.printf("Above %d: len=%d, hex=%s%n", boundary + 1, bytesAbove.length, hex.formatHex(bytesAbove));

      // Verify order preserved
      int objCmp = atBoundary.compareTo(aboveBoundary);
      int byteCmp = SirixDeweyID.compare(bytesAt, bytesAbove);

      assertTrue(objCmp < 0 && byteCmp < 0, "Order not preserved at tier boundary " + boundary);
    }
  }
  ...
  void verifyBijection() {
    ...
    String[] testCases = {"1", "1.0", "1.1", "1.126", "1.127", // Tier boundary - now fixed!
        "1.128", "1.3.5.7.9.11", "1.16510", "1.16511", // Tier boundary - now fixed!
        "1.16512", "1.2113662", "1.2113663", // Tier boundary - now fixed!
        "1.2113664"};
    ...
    for (String tc : testCases) {
      ...
      System.out.printf("%s -> encode -> decode: %s %s%n", tc, decoded.toString(), status);
  ...
  void verifyTierBoundaryFix() {
    ...
    for (int boundary : boundaries) {
      ...
      System.out.printf("1.%d -> encode -> decode: %s %s%n", boundary, decoded.toString(), status);
  ...
  void verifySpecialDivisionValues() {
    ...
    System.out.printf("RecordValue (1.0): %s%n", HexFormat.of().formatHex(recordValue.toBytes()));

    // Test attributeRoot (1)
    SirixDeweyID attrRoot = new SirixDeweyID("1.1");
    System.out.printf("AttributeRoot (1.1): %s%n", HexFormat.of().formatHex(attrRoot.toBytes()));
  ...
  void verifyDescendantRangeQuery() {
    ...
    for (String desc : descendants) {
      ...
      System.out.printf("  %s: %s, isPrefix=%b%n", desc, HexFormat.of().formatHex(dBytes), isPrefix);
    ...
    for (String nd : nonDescendants) {
      ...
      byte[] dBytes = d.toBytes();
      boolean isPrefix = isPrefixOf(parentBytes, dBytes);
      System.out.printf("  %s: %s, isPrefix=%b%n", nd, HexFormat.of().formatHex(dBytes), isPrefix);
    }
  }
