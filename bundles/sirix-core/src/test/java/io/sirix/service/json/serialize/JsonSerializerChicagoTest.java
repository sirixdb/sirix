...
/**
 * Integration tests for {@link JsonMaxLevelMaxNodesMaxChildNodesVisitor} using a chicago-like
 * dataset structure (object with "data" array containing many child arrays).
 */
public final class JsonSerializerChicagoTest {
  ...
  /**
   * Test serializing the "data" array with maxChildren limit. The data array contains 101 child
   * arrays (records). With maxChildren=5, we should get exactly 5 child arrays.
   */
  @Test
    ...
    try (final JsonResourceSession manager = database.beginResourceSession(JsonTestHelper.RESOURCE);
      ...
      try (final var rtx = manager.beginNodeReadOnlyTrx()) {
        ...
        try (final Writer writer = new StringWriter()) {
          final var serializer = new JsonSerializer.Builder(manager, writer).startNodeKey(dataArrayNodeKey)
                                                                            .maxLevel(2)
                                                                            .maxChildren(5)
                                                                            .build();
          serializer.call();
  ...
  /**
   * Test serializing the "data" array with maxChildren=50. Should return exactly 50 child arrays.
   */
  @Test
  ...
  /**
   * Count the number of top-level array children in a JSON array string. For example:
   * [[1,2],[3,4],[5,6]] has 3 children.
   */
