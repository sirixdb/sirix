 ...
 *
 * <p>
 * Run as: {@code HOTInvariantValidator.validate(rootRef, reader).assertOk()}.
 *
 * <p>
 * Each invariant is named explicitly so a regression points at a specific structural property.
 * Numbering matches {@code docs/HOT_INVARIANTS_CATALOG.md} (Stage A of the formal-verification
 * effort):
 * <ol>
 * <li><b>I1 — leaf-key-uniqueness</b>: every key within a leaf is unique; cross-leaf — no stored
 * key appears in two leaves.</li>
 * <li><b>I2 — leaf-lex-sorted</b>: leaf entries are stored in ascending lex order.</li>
 * <li><b>I3 — partial-key-uniqueness</b>: within an indirect node, no two children share a partial
 * key (would collide PEXT routing).</li>
 * <li><b>I4 — first-partial-zero</b>: under sparse-path encoding the leftmost child's stored
 * partial is zero (Binna's "first mask always zero" rule — every BiNode on the leftmost child's
 * path takes the LEFT side, so all on-path bits are 0).</li>
 ...
 * <li><b>I8 — children-sorted-by-firstkey</b>: child slots are also ordered by first-key (writer
 * enforces {@code sortChildrenByFirstKey}).</li>
 * <li><b>I9 — height-bounded</b>: trie height ≤ {@code maxHeight} (default 32 — generous upper
 * bound for ≤ 2³² entries).</li>
 * <li><b>I10 — fanout-bounded</b>: every indirect has ≥ 2 children and ≤
 * {@link HOTIndirectPage#MAX_NODE_ENTRIES} children.</li>
 * <li><b>I11 — trie-condition</b>: disc bits become strictly less significant down the tree, i.e.
 * for every parent → child indirect edge, every bit in {@code child.mask} has a larger absolute bit
 * position than {@code parent.MSB} (= no double-use of disc bits on a path; matches C++
 * {@code HOTSingleThreaded.hpp:521-523} {@code assert(parent.MSB <
 *       splitBit)}).</li>
 * <li><b>I-Binna-sparse-path</b>: per Binna §4.2 — stored partial p_i of child c_i must satisfy
 * {@code (p_i & ~PEXT(c_i.firstKey, mask)) == 0} (= every bit set in stored is also set in dense
 * PEXT of firstKey). Verified for firstKey only.</li>
 * <li><b>I-leaf-insert-precondition</b>: a STATIC helper, not a tree-walk. Given a hypothetical new
 * key being inserted, walks the PEXT-descent path and reports whether the insert would break I5 at
 * any ancestor (i.e., would a key with disagreeing β value be added to a previously β-constant
 * subtree). Used by Stage D's post-mutation gate to surface the leaf-insert I5 gap at the operation
 * that creates it. See {@link #checkLeafInsertPreservesI5}.</li>
 * </ol>
 *
 * <p>
 * Each violation is collected as a {@link Violation} so the caller can report all problems in one
 * pass instead of failing at the first one.
 */
public final class HOTInvariantValidator {
  ...
  /**
   * Validate a HOT trie rooted at {@code rootRef}. Returns a result that can be queried for
   * violations or asserted via {@link Result#assertOk()}.
   */
   ...
   * {@link io.sirix.index.hot.AbstractHOTIndexReader#getRootReference()}. Returns {@code null} if the
   * index has not been materialized for the current revision.
   */
  public static @Nullable PageReference resolveRootRef(StorageEngineReader reader, IndexType indexType,
      int indexNumber) {
  ...
  public static Result validateIndex(StorageEngineReader reader, IndexType indexType, int indexNumber) {
  ...
  private void walk(Page page, int depth, List<HOTIndirectPage> ancestorIndirects) {
    ...
    if (depth > observedHeight)
      observedHeight = depth;
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (childRef == null) {
        addViolation("null-child", "indirect[" + indirect.getPageKey() + "].child[" + i + "] = null", indirect);
        continue;
      }
      ...
      if (child == null) {
        ...
        addViolation("unloadable-child",
            "indirect[" + indirect.getPageKey() + "].child[" + i + "] could not be loaded" + " (logKey="
                + childRef.getLogKey() + " gen=" + childRef.getActiveTilGeneration() + " diskKey=" + childRef.getKey()
                + " rawPage=" + (rawPage == null
                    ? "null"
                    : rawPage.getClass().getSimpleName() + (rawPage.isClosed()
                        ? "-CLOSED"
                        : "-OPEN"))
                + ")",
            indirect);
  ...
  /**
   * I12 — subtree-ranges-disjoint. Each child's subtree must own a CONTIGUOUS, non-overlapping span
   * of key space, and those spans must ascend with the child slot.
   *
   * <p>
   * This is the trie property itself, stated in terms a walk can check: in Binna's HOT a node's
   * discriminative bits are exactly the bits on which its subtree's keys differ, so a partial key is
   * an ORDER-PRESERVING encoding and each child owns one interval. The moment the mask misses a
   * discriminating bit, two keys that differ only on the missing bit collapse to the same partial key
   * and land in the same child — and a third key that sorts BETWEEN them routes elsewhere. The
   ...
   * row groups {@code 128..159} AND {@code 192..196} while the NEXT child held {@code 160..191} — the
   * node discriminated on bit 5 while ignoring the more significant bit 6.
   */
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (previousMax != null && Arrays.compareUnsigned(previousMax, range[0]) >= 0) {
        addViolation("I12-subtree-ranges-disjoint",
            "indirect " + indirect.getPageKey() + ": child[" + previousSlot + "] spans up to " + hex(previousMax)
                + " but child[" + i + "] starts at " + hex(range[0])
   ...
   *
   * <p>
   * Deliberately takes the min/max over ALL descendants rather than the leftmost/rightmost leaf:
   * assuming the leftmost leaf holds the minimum would assume the very ordering property this check
   * exists to test.
   */
  ...
  private void validateI11TrieCondition(HOTIndirectPage parent, HOTIndirectPage child, int childSlot) {
    ...
    if (parentMSB < 0)
      return; // parent's MSB undefined — skip
    final int childMSB = child.getMostSignificantBitIndex();
    if (childMSB < 0)
      return; // child has no disc bits — vacuously OK (rare)
  ...
  private void validateLeaf(HOTLeafPage leaf, List<HOTIndirectPage> ancestorIndirects) {
    ...
    if (pageKey >= 0)
      visitedLeafPageKeys.add(pageKey);
  ...
  private void validateIndirect(HOTIndirectPage indirect) {
    ...
    // I10 — fanout-bounded.
    if (n < 2) {
      addViolation("I10-fanout-bounded", "indirect " + indirect.getPageKey() + " has " + n + " children (< 2)",
          indirect);
    }
    if (n > HOTIndirectPage.MAX_NODE_ENTRIES) {
      addViolation("I10-fanout-bounded",
          "indirect " + indirect.getPageKey() + " has " + n + " children (> " + HOTIndirectPage.MAX_NODE_ENTRIES + ")",
          indirect);
    }
    ...
    if (partials != null && partials.length >= n) {
      ...
      for (int i = 0; i < n; i++) {
        if (!seen.add(partials[i])) {
          addViolation("I3-partial-key-uniqueness", "indirect " + indirect.getPageKey()
              + " has duplicate partial key 0x" + Integer.toHexString(partials[i]) + " across children", indirect);
          break;
        }
      }
    }
      ...
      for (int i = 1; i < n; i++) {
        if (Integer.compareUnsigned(partials[i], partials[i - 1]) <= 0) {
          addViolation("I7-partial-keys-sorted",
              "indirect " + indirect.getPageKey() + " partial keys not ascending at " + (i - 1) + "→" + i + ": 0x"
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (previousFirstKey != null) {
        ...
        if (cmp >= 0) {
          ...
          dump.append(" mask=0x")
              .append(Long.toHexString(indirect.getBitMask()))
              .append(" initialBytePos=")
              .append(indirect.getInitialBytePos())
              .append(" dump=[");
          for (int k = 0; k < n; k++) {
            final byte[] fk = firstKeyOfSubtree(indirect.getChildReference(k));
            final int densePKk = fk == null
                ? 0
                : computeDensePartialKey(indirect, fk);
            dump.append("c")
                .append(k)
                .append("(sparse=0x")
                .append(partials != null && partials.length > k
                    ? Integer.toHexString(partials[k])
                    : "?")
                .append(",dense=0x")
                .append(Integer.toHexString(densePKk))
                .append(",fk=")
                .append(fk == null
                    ? "null"
          ...
          addViolation("I8-children-sorted-by-firstkey",
              "indirect " + indirect.getPageKey() + " child[" + (i - 1) + "].firstKey " + bytesHex(previousFirstKey)
                  + " >= child[" + i + "].firstKey " + bytesHex(firstKey) + dump,
              indirect);
          break;
        }
      }
    ...
    if (partials != null && partials.length >= n) {
      for (int i = 0; i < n; i++) {
        ...
        if ((sparsePK & ~densePK) != 0) {
          ...
          final StringBuilder dump = new StringBuilder(256);
          dump.append(" mask-popcount=")
              .append(Long.bitCount(indirect.getBitMask()))
              .append(" initialBytePos=")
              .append(indirect.getInitialBytePos())
              .append(" mask=0x")
              .append(Long.toHexString(indirect.getBitMask()))
          ...
          for (int k = 0; k < n; k++) {
            ...
            final Page cPage = cref == null
                ? null
                : loadPage(cref);
            final String pageKind = cPage == null
                ? "null"
                : cPage instanceof HOTLeafPage cl
                    ? "leaf(" + cl.getPageKey() + ",ec=" + cl.getEntryCount() + ")"
                    : cPage instanceof HOTIndirectPage ci
                        ? "ind(" + ci.getPageKey() + ",nc=" + ci.getNumChildren() + ")"
                        : cPage.getClass().getSimpleName();
            dump.append("c")
                .append(k)
                .append("(sparse=0x")
                .append(Integer.toHexString(sparsePKk))
                .append(",dense=0x")
                .append(Integer.toHexString(densePKk))
                .append(",pg=")
                .append(pageKind)
                ...
                .append(fk == null
                    ? "null"
                    : bytesHex(fk).substring(0, Math.min(48, bytesHex(fk).length())))
                .append(",lastK=")
                .append(lastKeyOfChildHex(cref))
                .append(") ");
          }
          dump.append("]");
          addViolation("I-Binna-sparse-path",
              "indirect " + indirect.getPageKey() + " child[" + i + "] stored partial 0x"
                  + Integer.toHexString(sparsePK) + " has bits not in dense PEXT 0x" + Integer.toHexString(densePK)
                  + " — sparse-path-encoded partials must satisfy (sparse & ~dense) == 0" + dump,
              indirect);
  ...
  private void validateLeafBetaConstancy(final HOTIndirectPage indirect, final int n, final int @Nullable [] partials) {
    if (partials != null && partials.length >= n) {
      for (int i = 0; i < n; i++) {
        final int sparsePK = partials[i];
        if (sparsePK == 0)
          continue; // no ON-PATH bit to verify
        ...
        if (failingKey[0] != null) {
          addViolation("I5-leaf-constancy",
              "indirect " + indirect.getPageKey() + " child[" + i + "] stored partial 0x"
                  ...
                  + Integer.toHexString(failingKeyDensePK[0]) + " of subtree key " + bytesHex(failingKey[0])
                  + " — at least one key in c's subtree fails" + " the (sparse & ~dense) == 0 condition",
              indirect);
  ...
  private void verifyPextRouting(PageReference rootRef) {
    ...
    for (final byte[] key : storedKeys) {
      ...
      if (routed == null) {
        addViolation("I6-pext-routes-to-leaf", "PEXT descent for stored key " + bytesHex(key) + " returned null leaf",
            null);
        continue;
      }
      if (routed.findEntry(key) < 0) {
        ...
        addViolation("I6-pext-routes-to-leaf", "PEXT descent for stored key " + bytesHex(key) + " landed in leaf "
            + routed.getPageKey() + " which does not contain it", null);
      }
    }
  }
  ...
  /**
   * Diagnostic: full-scan every reachable leaf for {@code key} and return its {@code pageKey}, or
   * {@code -1} if no leaf contains it. Used by the I6 trace to identify where a misrouted stored key
   * actually lives.
   */
   ...
   * actually leads to the target leaf. Used to compare PEXT routing vs. actual leaf location for I6
   * violations.
   */
  ...
  private boolean tracePathToLeafRec(PageReference ref, long targetLeafKey, byte[] key, int depth, StringBuilder sb) {
    if (depth > maxHeight + 1)
      return false;
    final Page page = loadPage(ref);
    if (page instanceof HOTLeafPage leaf) {
    ...
    for (int i = 0; i < indirect.getNumChildren(); i++) {
      ...
      if (tracePathToLeafRec(childRef, targetLeafKey, key, depth + 1, sb)) {
        ...
        sb.append("    depth=")
          .append(depth)
          .append(" indirect=")
          .append(indirect.getPageKey())
          .append(" densePK=0x")
          .append(Integer.toHexString(densePK))
          .append(" → child[")
          .append(i)
          .append("] partial=0x")
  ...
  /**
   * Diagnostic: traces the PEXT-descent path for {@code key} from {@code startRef}. At each indirect
   * level emits {@code pageKey, mask popcount, dense PEXT of key, child index chosen,
   * child's stored partial}. Used to debug I6 violations — call from the I6 check site when a stored
   * key fails to route to its actual leaf.
   */
  String tracePextDescend(PageReference startRef, byte[] key) {
    ...
    for (int depth = 0; depth <= maxHeight; depth++) {
      final Page page = loadPage(ref);
      if (page == null) {
        sb.append("  depth=").append(depth).append(" → null\n");
        break;
      }
      if (page instanceof HOTLeafPage leaf) {
        sb.append("  depth=")
          .append(depth)
          .append(" leaf=")
          .append(leaf.getPageKey())
          .append(" ec=")
          .append(leaf.getEntryCount())
          .append(" contains=")
          .append(leaf.findEntry(key) >= 0)
          .append("\n");
        break;
      }
      if (!(page instanceof HOTIndirectPage indirect)) {
  ...
  /**
   * Static check: would inserting {@code newKey} into the trie rooted at {@code rootRef} violate I5
   * (leaf β-constancy) at any ancestor on the PEXT-descent path?
   *
   * <p>
   * Algorithm: walk PEXT-descent path. At each ancestor A:
   * <ul>
   * <li>Compute the chosen child slot c via {@code A.findChildIndex(newKey)}.</li>
   * <li>For every disc bit β captured by A's mask: scan c's subtree for existing keys' β-bit values.
   * If existing keys agree on a single value v ∈ {0,1} but {@code newKey.β != v}, report a violation:
   * inserting newKey would change c's subtree from β-constant to β-mixed.</li>
   * <li>Mixed existing values do NOT trigger a violation — they indicate either an already-broken I5
   * (separately reported by I5-leaf-constancy on full validation) or sparse-path off-path bits (legal
   * under Binna's encoding).</li>
   * </ul>
   *
   * <p>
   * This surfaces the campaign's recurring root cause from {@code
   * HOT_OPERATIONS_INVARIANTS_MATRIX.md} §5.4: Op 1 (leaf.put) inserts a key with disagreeing β value
   * into a previously β-constant subtree, silently breaking I5. Routing soundness alone doesn't
   * prevent it because subset-match routing {@code (dense & stored) == stored} only enforces a
   * one-sided constraint.
   *
   * <p>
   * <b>Performance.</b> One full subtree scan per ancestor disc bit. Acceptable for Stage D's gated
   * post-mutation check on the 50K reproducer; not appropriate for production paths.
   *
  ...
  public static List<Violation> checkLeafInsertPreservesI5(PageReference rootRef, byte[] newKey,
    ...
    if (rootRef == null || newKey == null || reader == null)
      return List.of();
  ...
  /**
   * Per-ancestor logic for {@link #checkLeafInsertPreservesI5}. Iterates each absolute disc bit
   * position β captured by {@code indirect}'s mask (handling both SingleMask and MultiMask layouts),
   * runs {@link #scanSubtreeBetaValue} over the chosen child's subtree, and emits a violation when
   * subtree is β-constant at value v but {@code newKey.β != v}.
   */
  private void checkPreconditionAtIndirect(HOTIndirectPage indirect, int chosenIdx, PageReference chosenRef,
      byte[] newKey) {
    ...
    for (int i = 0; i < numExtractionBytes; i++) {
      ...
      // The 8 bits of this byte sit at long-bits ((7-byteOffsetInChunk)*8) .. +7 in
  ...
  private void evaluateBetaPrecondition(HOTIndirectPage indirect, int chosenIdx, PageReference chosenRef, byte[] newKey,
      int absBit) {
    final int newKeyBetaValue = isBitSetAbsolute(newKey, absBit)
        ? 1
        : 0;
    ...
    if (subtreeBetaState == 0 && newKeyBetaValue != 0) {
      addViolation("I-leaf-insert-precondition",
          "indirect " + indirect.getPageKey() + " child[" + chosenIdx + "]'s subtree is β=0-constant" + " at absBit="
              + absBit + " but newKey has β=1 — insert would turn subtree β-mixed",
          indirect);
    } else if (subtreeBetaState == 1 && newKeyBetaValue != 1) {
      addViolation("I-leaf-insert-precondition",
          "indirect " + indirect.getPageKey() + " child[" + chosenIdx + "]'s subtree is β=1-constant" + " at absBit="
              + absBit + " but newKey has β=0 — insert would turn subtree β-mixed",
          indirect);
    }
  ...
  /**
   * Scan every leaf reachable from {@code ref}, evaluate each key's bit at absolute MSB-first
   * position {@code absBit}, return:
   * <ul>
    ...
    walkLeavesUntilFalse(ref, leaf -> {
      ...
      for (int k = 0; k < ec; k++) {
        ...
        if (isBitSetAbsolute(key, absBit))
          ...
          seen0[0] = true;
        if (seen0[0] && seen1[0])
          return false; // early exit on mixed
      }
      return true;
    });
    if (seen0[0] && seen1[0])
  ...
  /**
   * MSB-first absolute bit lookup: bit at absolute position {@code absBit} = byte {@code absBit/8},
   * bit-within-byte {@code 7-(absBit%8)} (so {@code absBit==0} = MSB of {@code key[0]}). Returns
   * {@code false} for out-of-range positions (= bit is implicit zero past the key length).
   */
   ...
   * {@link io.sirix.page.HOTIndirectPage}'s SingleMask / MultiMask partial-key extraction so the
   * validator can compare stored sparse-path partials against the would-be dense PEXT.
   */
  ...
  /**
   * Cheap "last key in subtree" — walks rightmost path. Bounded by maxHeight. Returns hex (truncated
   * to 48 chars) for diagnostic dumps.
   */
  ...
  private void addViolation(String invariant, String message, @Nullable HOTIndirectPage parent) {
    violations.add(new Violation(invariant, message, parent == null
        ? -1L
        : parent.getPageKey()));
  }
  ...
  public record Violation(String invariant, String message, long parentPageKey) {
    ...
    public String toString() {
      return "[" + invariant + "] " + message + (parentPageKey >= 0
          ? " (parent=" + parentPageKey + ")"
          : "");
    }
  }
   ...
   *
   * <p>
   * <b>UPDATED 2026-05-12:</b> Now EMPTY. Previously contained I3, I6, I7 — those were "soft" because
   * of the HOT augment-bailout limitation in `createNodeFromChildren`. After the structural-Binna
   * campaign (Phases 5–7q + Path 5 routing-collision check, default-ON via `hot.strict.{g32, phase7q,
   * g32.deep, g32.multibeta, g32.childmsb, path5.routeverify} = true`), the augment-bailout no longer
   * surfaces these violations on conforming workloads. They are promoted to HARD violations — any
   * occurrence is a real correctness gap.
   * </p>
   *
   * <p>
   * Kept as a tag list (rather than deleted) so {@link #assertNoHardViolations()} and
   * {@link #hardViolations()} remain API-stable; callers that want the old lenient behavior can pass
   * an explicit override set if they need to characterize a pre-fix workload.
   * </p>
   */
  ...
  public static final class Result {
    ...
    /**
     * Violations that are NOT in the structural-limitation set — i.e., must hold for any HOT
     * correctness claim.
     */
    public List<Violation> hardViolations() {
      return violations.stream().filter(v -> !STRUCTURAL_LIMITATION_INVARIANTS.contains(v.invariant())).toList();
    }
    ...
    /**
     * Assert no violations were found. Throws {@link AssertionError} listing every violation if any
     * were collected.
     */
    public void assertOk() {
      if (violations.isEmpty())
        return;
      final StringBuilder sb = new StringBuilder("HOT invariant violations (").append(violations.size()).append("):\n");
     ...
     * {@link #STRUCTURAL_LIMITATION_INVARIANTS} are merely reported via the returned summary. Use for
     * adversarial workloads where the documented HOT-augment-bailout limitation may surface I3 / I6 /
     * I7 violations that don't affect production reads.
     */
    public String assertNoHardViolations() {
      ...
      if (!hard.isEmpty()) {
        final StringBuilder sb =
            new StringBuilder("HOT HARD invariant violations (").append(hard.size()).append("):\n");
        for (final Violation v : hard)
          sb.append("  ").append(v).append('\n');
        if (!soft.isEmpty()) {
          sb.append("Plus ")
            .append(soft.size())
