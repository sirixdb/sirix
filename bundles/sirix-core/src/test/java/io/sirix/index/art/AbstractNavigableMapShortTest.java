...
//
/// **
// * When number of sample keys are very large, we need to avoid O(n^2) loops to
// keep the test runtime short.
// * For example tests that call AbstractMapTest.verify() inside a loop.
// */
// public abstract class AbstractNavigableMapShortTest<K, V> extends
// AbstractNavigableMapTest<K, V> {
...
// at
// java.util.AbstractCollection.addAll(java.base@12.0.2/AbstractCollection.java:352)
// at java.util.HashSet.<init>(java.base@12.0.2/HashSet.java:120)
// at
// org.apache.commons.collections4.map.AbstractMapTest.testEntrySetRetainAll(AbstractMapTest.java:1473)
// */
// }
//
// @Override
...
// assertNotNull("failure in test: Must have keys returned from getSampleKeys.",
// keys);
// assertNotNull("failure in test: Must have values returned from
// getSampleValues.", values);
// assertEquals("failure in test: not the same number of sample keys and
// values.", keys.length, values.length);
// assertEquals("failure in test: not the same number of values and new
// values.", values.length, newValues.length);
//
// for (int i = 0; i < keys.length - 1; ++i) {
// /*for(int j = i + 1; j < keys.length; ++j) {
// assertTrue("failure in test: duplicate null keys.", keys[i] != null ||
// keys[j] != null);
// assertTrue("failure in test: duplicate non-null key.", keys[i] == null ||
// keys[j] == null || !keys[i].equals(keys[j]) && !keys[j].equals(keys[i]));
// }*/
//
// assertTrue("failure in test: found null key, but isNullKeySupported is
// false.",
// keys[i] != null || this.isAllowNullKey());
// assertTrue("failure in test: found null value, but isNullValueSupported is
// false.",
// values[i] != null || this.isAllowNullValue());
// assertTrue("failure in test: found null new value, but isNullValueSupported
// is false.",
// newValues[i] != null || this.isAllowNullValue());
// assertTrue("failure in test: values should not be the same as new value",
// values[i] != newValues[i] && (values[i] == null ||
...
// this.getMap().containsKey(keys[i]));
// // assertTrue("Map should contain value after put",
// this.getMap().containsValue(values[i]));
// }
//
...
// // this.verify();
// this.verifyShort();
// assertEquals("Map.put should return previous value when changed", values[i],
// o);
...
// fail("Expected IllegalArgumentException or UnsupportedOperationException on
// put (change)");
...
// fail("Expected UnsupportedOperationException or IllegalArgumentException on
// put (add) when fixed size");
...
// i = 0;
//
// for (Iterator<K> it = this.getMap().keySet().iterator(); it.hasNext() && i <
// newValues.length; ++i) {
// K key = it.next();
...
// assertEquals("map.remove for nonexistent key should not shrink map", size,
// this.getMap().size());
// }
//
// this.verify();
