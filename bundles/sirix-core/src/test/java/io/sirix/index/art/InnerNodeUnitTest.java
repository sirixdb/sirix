...
public abstract class InnerNodeUnitTest {
  ...
  public void setup() {
    ...
    assertTrue(i < existingData.length,
        "sample key set should contain at least" + " one negative integer to test for unsigned lexicographic ordering");
  }
  ...
  /*
   * work only with interface methods we don't care about the implementation details for example how
   * Node4 stores bytes as unsigned, etc. we just care about the right lexicographic ordering. of
   * course this requires us to test with negative as well as positive data set and hence the check in
   * test setup. we don't test child mappings, since that is tested in findChild already (if the same
   * mappings are maintained). this really is making sure negative bytes come after positives. we
   * don't really want to test that children storage is sorted, all we want is if the lexicographic
   * order dependant methods (first, last, greater, lesser) are answered correctly. they might be
   * answered correctly even without storing the children in sorted order, but we don't care as a
   * generic test suite. we base our assertions on invariants.
   */
  ...
  /*
   * add partial keys all key, child mappings should exist size increase uplinks setup expect keys to
   * be in the right unsigned lexicographic order
   */
  @Test
  ...
  /*
   * nothing greater than greatest first is greater than smallest lexicographic unsigned i.e. 0 (0000
   * 0000)
   */
  @Test
  ...
  /*
   * nothing lesser than least last is lesser than largest lexicographic unsigned i.e. -1 (1111 1111)
   */
  @Test
  ...
  /*
   * remove child unsigned lexicopgrahic order maintained removes uplink reduces size child no longer
   * exists (findChild)
   */
  @Test
  ...
  /*
   * after growing, new node: contains same key, child mappings in same lexicographic order but with
   * uplinks to new grown node same prefix key, no of children, uplink key, parent
   */
  @Test
  ...
  /*
   * after shrinking contains same key, child mappings lexicographic order maintained same parent as
   * before, prefix len, prefix keys
   */
  @Test
  ...
  /*
   * replace the child associated with a key assert new child found same size lexicographic order
   * maintained uplink setup for new child old child uplink stays: why? because in lazy leaf expansion
   * case, we first link current leaf node with a new Node4() and later replace current down pointer
   * to this leaf node with this new Node4() parent. If we remove old child's uplink, it could be the
   * case that the old child has been linked with a new parent. Well we could make sure that
   * explicitly in the branch, but it is fine to not do in replace as well.
   */
  @Test
