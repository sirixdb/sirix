...
public class ARTUnitTest {
  ...
  /*
   * cp for all i == key for all i but len(key) >= len(cp) expect 0 but len(key) < len(cp) expect 1 cp
   * at i < key at i expect -1 cp at i > key at i expect 1
   */
  @Test
  ...
  /*
   * cover all windows (toCompress, linking key, onlyChild) everything from toCompress everything from
   * toCompress + linking key everything from toCompress + linking key + some from child everything
   * from toCompress + linking key + all from child
   */
  @Test
  ...
  public void testBranchOutPessimistic() {
    ...
    node.prefixLen = 10;
    Assertions.assertThrows(AssertionError.class, () -> AdaptiveRadixTree.branchOutPessimistic(node, bc.get(key), key,
        value, InnerNode.PESSIMISTIC_PATH_COMPRESSION_LIMIT, 5));
  ...
  public void testReplace() {
    ...
    // replace root's x downlink with new child (for various reasons, for example because we just grew
    // this child)
