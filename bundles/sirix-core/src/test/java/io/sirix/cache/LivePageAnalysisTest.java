...
public class LivePageAnalysisTest {
  ...
  public void testAnalyzeLivePages() {
    ...
    try {
      ...
      long liveInRecordCache =
          liveSnapshot.stream().filter(p -> bufferManager.getRecordPageCache().asMap().containsValue(p)).count();

      long liveInFragmentCache =
          liveSnapshot.stream()
      ...
      if (swizzledOnlyPinned > 0) {
        ...
        liveSnapshot.stream()
                    ...
                    .filter(p -> false) // p.getPinCount() > 0) // REMOVED
                    .limit(10)
                    .forEach(p -> System.err.println("    Page " + p.getPageKey() + " (rev " + p.getRevision()
                        + ", type " + p.getIndexType() + ")"));
