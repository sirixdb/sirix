	...
	private void test(@Nonnull final String FOLDER) throws Exception {
		...
		Collections.sort(list, new Comparator<File>() {
			...
			public int compare(final File paramFirst, final File paramSecond) {
				final String firstName = paramFirst.getName().toString()
						.substring(0, paramFirst.getName().toString().indexOf('.'));
				final String secondName = paramSecond.getName().toString()
						.substring(0, paramSecond.getName().toString().indexOf('.'));
		...
		for (final File file : list) {
			if (file.getName().endsWith(".xml")) {
				if (first) {
					...
					FMSEImport
							.main(new String[] { PATHS.PATH1.getFile().getAbsolutePath(),
