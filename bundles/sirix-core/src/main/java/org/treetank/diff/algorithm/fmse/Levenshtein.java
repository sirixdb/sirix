...
import org.treetank.diff.algorithm.fmse.utils.ISubstitutionCost;
import org.treetank.diff.algorithm.fmse.utils.SubCost01;
...
public final class Levenshtein {
   ...
   */
  public float getSimilarityTimingEstimated(@Nonnull final String pFirst, @Nonnull final String pSecond) {
    // timed millisecond times with string lengths from 1 + 50 each increment
    // 0 0.31 1.12 2.4 4.41 6.77 11.28 14.5 24.33 31.29 43.6 51 54.5 67.67 68 78 88.67 101.5 109 117.5
    // 140.5 148.5 156 180 187.5 219 203 250 250 312 297 328 297 359 360 406 453 422 437 469 500 516 578
   ...
   * {@link Levenshtein#MIN} and {@link Levenshtein#MAX}. Otherwise string equality is used whereas {@code 0}
   * is returned if the strings aren't equal and {@code 1} if they are equal.
   * 
   * @param pFirst
   *          first string
   * @param pSecond
   *          second string
   * @return a value between {@code 0} and {@code 1}. {@code 0} denotes that the strings are completely
   *         different, {@code 1} denotes that the strings are equal
   * @throws NullPointerException
   *           if {@code pFirst} or {@code pSecond} is {@code null}
   */
  public float getSimilarity(@Nonnull final String pFirst, @Nonnull final String pSecond) {
    checkNotNull(pFirst);
   ...
   * Implements the levenstein distance function.
   * 
   * Copy character from string1 over to string2 (cost 0) Delete a character in string1 (cost 1) Insert a
   * character in string2 (cost 1) Substitute one character for another (cost 1)
   * 
   * <pre>
   ...
   */
  private float getUnNormalisedSimilarity(@Nonnull final String s, @Nonnull final String t) {
    assert s != null;
    assert t != null;
