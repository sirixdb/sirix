...
public class AmazonS3StorageWriter extends AbstractForwardingReader implements Writer {
	...
	public AmazonS3StorageWriter(final String dataFileKeyName, final String revisionsOffsetFileKeyName,
			final String bucketName, final SerializationType serializationType, final PagePersister pagePersister,
		...
		try {
			this.dataFile = new RandomAccessFile(requireNonNull(reader.readObjectDataFromS3(dataFileKeyName)).toFile(),
					"rw");
			this.revisionsFile = type == SerializationType.DATA
					? new RandomAccessFile(
							requireNonNull(reader.readObjectDataFromS3(revisionsOffsetFileKeyName)).toFile(), "rw")
					: null;
		} catch (IOException io) {
			LOGGER.error(String.format(
					"Cannot create S3 storage writer, "
							+ "please check if DATA path OR Revision offset file path exists. Error details : %s",
					io.getMessage()));
		}
	 ...
	 * @param keyName    - Name of the file that includes the full path that is
	 *                   supposed to be used on the local file system
	 * @param object     - File that could be read from the local filesystem that
	 *                   contains the actual information to be stored on S3
	 * 
	 */
	protected void writeObjectToS3(String keyName, File object, boolean isDataFile) {
		try {
			...
			PutObjectRequest putOb = PutObjectRequest.builder().bucket(bucketName).key(keyName).metadata(metadata)
					.build();
			...
			objectFutureResponse.whenComplete((response, error) -> {
				try {
					if (response != null) {
						...
						/*
						 * No need to delete/cleanup the file as we are writing on the local file
						 * system, so this avoid unnecessarily filling up the filesystem space
						 */
					} else {
						// Handle error
	...
	private String getFileKeyName(String fileDescriptorPath) {
		return fileDescriptorPath
				.substring((System.getProperty("java.io.tmpdir") + FileSystems.getDefault().getSeparator()).length());
	}

	@NotNull
	private AmazonS3StorageWriter writePageReference(final PageReadOnlyTrx pageReadOnlyTrx,
		...
		try {
			...
			try (final ByteArrayOutputStream output = new ByteArrayOutputStream(1_000);
					final DataOutputStream dataOutput = new DataOutputStream(
							reader.getByteHandler().serialize(output))) {
			...
			this.writeObjectToS3(this.getFileKeyName(dataFile.getFD().toString()),
					new File(dataFile.getFD().toString()), Boolean.TRUE);
			...
			if (type == SerializationType.DATA) {
				...
				this.writeObjectToS3(this.getFileKeyName(revisionsFile.getFD().toString()),
						new File(revisionsFile.getFD().toString()), Boolean.FALSE);
			}
	...
	public Writer truncateTo(PageReadOnlyTrx pageReadOnlyTrx, int revision) {
		try {
			final var dataFileRevisionRootPageOffset = cache.get(revision, (unused) -> getRevisionFileData(revision))
			...
			this.writeObjectToS3(getFileKeyName(dataFile.getFD().toString()), new File(dataFile.getFD().toString()),
					Boolean.TRUE);
	...
	public Writer truncate() {
		try {
			...
			if (revisionsFile != null) {
				...
				this.writeObjectToS3(getFileKeyName(revisionsFile.getFD().toString()),
						new File(revisionsFile.getFD().toString()), Boolean.FALSE);
			}
		} catch (final IOException e) {
			throw new SirixIOException(e);
