...
/**
 * Retrieve a node by node key in all revisions. In each revision a {@link XmlNodeReadOnlyTrx} is opened
 * which is moved to the node with the given node key if it exists. Otherwise the iterator has no
 * more elements (the {@link XmlNodeReadOnlyTrx} moved to the node by it's node key).
 *
