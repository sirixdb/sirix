...
public final class IOUringStorage implements IOStorage {
	...
	private static final EventExecutor eventExecutor = EventExecutor.builder().entries(1024).sleepTimeout(10).build();
	...
	private void createDataFileIfNotInitialized(Path dataFilePath) {
		CompletableFuture<AsyncFile> asyncFileCompletableFuture = AsyncFile.open(dataFilePath, eventExecutor,
				OpenOption.READ_WRITE, OpenOption.CREATE);
	...
	public Writer createWriter() {
		try {
			...
			return new IOUringWriter(dataFile, revisionsOffsetFile, dataFilePath, revisionsOffsetFilePath,
					serializationType, pagePersister, cache, reader);
		} catch (final IOException | InterruptedException e) {
			throw new SirixIOException(e);
		} finally {
			semaphore.release();
