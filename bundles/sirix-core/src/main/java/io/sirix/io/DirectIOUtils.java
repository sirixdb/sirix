...
public final class DirectIOUtils {
	...
	/**
	 * Allocates a block aligned direct byte buffer. The size of the returned buffer
	 * is the nearest multiple of BLOCK_SIZE to the requested size.
	 *
	 * @param size
	 *            the requested size of a byte buffer.
	 ...
	 * <p>
	 * We rely on ByteBuffer.compact() method so that the caller of this method can
	 * safely flip() the buffer for reading. Avoiding compact() improves performance
	 * but requires callers of this method to avoid using flip() and not rely on a
	 * position-0 invariant for chunk reader.
	 *
	 * @param channel
	 ...
	 * @param position
	 *            the position of the channel to start reading from.
