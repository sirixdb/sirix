...
/**
 * Striped buffer pool for parallel serialization (ScyllaDB pattern). Reduces contention by
 * partitioning buffers across CPU cores.
 * 
 * <p>
 * This pool is designed for use during parallel page serialization in
 * {@code NodeStorageEngineWriter.parallelSerializationOfKeyValuePages()}. It eliminates the
 * overhead of creating new Arena.ofAuto() instances for each page serialized.
 * </p>
 * 
 * <p>
 * The pool uses striping to reduce lock contention: each CPU core has its own deque of buffers, so
 * threads rarely contend for the same lock.
 * </p>
 * 
 * <p>
 * Thread safety: This class is thread-safe. Each stripe's deque is accessed under its own lock, and
 * the stripe assignment is based on thread ID.
 * </p>
 * 
...
public final class SerializationBufferPool {
  ...
  public SerializationBufferPool(int bufferSize) {
    ...
    LOGGER.debug("SerializationBufferPool initialized: {} stripes × {} buffers = {} total, {}KB each", STRIPE_COUNT,
        BUFFERS_PER_STRIPE, STRIPE_COUNT * BUFFERS_PER_STRIPE, bufferSize / 1024);
  }
   ...
   * 
   * <p>
   * If the thread's stripe has available buffers, one is returned immediately. If the stripe is
   * exhausted, a temporary buffer is allocated (not from pool).
   * </p>
   * 
   ...
   * 
   * <p>
   * The buffer is reset (clearing any overflow arena) before being returned. If the stripe is full,
   * the underlying segment is returned to the allocator.
   * </p>
   * 
  ...
  /**
   * Get the stripe index for the current thread. Uses thread ID for consistent assignment.
   */
  ...
  public String getStats() {
    ...
    return String.format("SerializationBufferPool: %d/%d buffers available across %d stripes", total,
        STRIPE_COUNT * BUFFERS_PER_STRIPE, STRIPE_COUNT);
  }
