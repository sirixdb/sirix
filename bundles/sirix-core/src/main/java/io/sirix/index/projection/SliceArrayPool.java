 ...
 *
 * <p>
 * A windowed group-by pass decodes every leaf of every needed column into a fresh {@code long[rowCount]}
 * plus its presence words, keeps the slice for two windows in the access's LRU and drops it — at 100M a
 * four-column composite pass minted 3.2 GB of such arrays PER PASS (q32: 8 passes × 97,654 leaves × 4
 * columns × 8 KB), all of it dying young and driving 80+ collections per try. The lifetime is known
 * exactly: an array is dead the moment its slice leaves the LRU, so the access hands the evicted slice
 * here and the next window's decode takes the arrays back instead of allocating. Not thread-safe — one
 * pool per access, like the access itself.
 * </p>
 *
 * <p>
 * <b>Exact lengths only.</b> Consumers read {@code numericValues().length} as the row count, so a
 * recycled array is handed out only for a request of its exact length; a mismatch (the store's last,
 * short leaf) allocates. Both lanes are single stacks whose top is popped on a length match and dropped
 * otherwise — every leaf but one has {@link ProjectionIndexRowGroupPage#MAX_ROWS} rows, so a stack of
 * uniform lengths is the common case and the odd length costs one allocation.
 * </p>
 *
 * <p>
 * <b>Only the decoders zero what they need.</b> {@code decodePresenceInto} fills every word in every
 * mode and the FOR/ALP decoders overwrite every cell, so a recycled array carries no stale state into
 * a slice. A decoder that writes only "the set bits" must not be handed one.
 * </p>
 */
final class SliceArrayPool {
   ...
   * hands out are taken: presence words and numeric values; dictionary and boolean lanes are untouched.
   * The caller guarantees no consumer still reads the slice — the windowed contract (fill, kernel,
   * release, then the next window) is what makes eviction that guarantee.
   */
