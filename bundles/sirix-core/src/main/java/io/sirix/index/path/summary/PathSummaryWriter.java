...
public final class PathSummaryWriter<R extends NodeCursor & NodeReadOnlyTrx>
		extends
			AbstractForwardingPathSummaryReader {
	...
	public enum OPType {
		/**
		 * Move from and to is on the same level (before and after the move, the node
		 * has the same parent).
		 */
		MOVED_ON_SAME_LEVEL,
	 ...
	 * @param nodeFactory
	 *            The node factory to create path nodes
	 * @param rtx
	 *            the read-only trx
	 */
	...
	public long getPathNodeKey(final QNm name, final NodeKind pathKind) {
		...
		final Axis axis = new FilterAxis<>(new ChildAxis(pathSummaryReader),
				new PathNameFilter(pathSummaryReader,
						pathKind == NodeKind.NAMESPACE ? name.getPrefix() : Utils.buildName(name)),
	...
	/**
	 * Move path summary cursor to the path node which is references by the current
	 * node.
	 */
	...
	private void adaptForInsert(final Node newNode) {
		...
		if (newNode instanceof StructNode strucNode) {
			final StructNode parent = pageTrx.prepareRecordForModification(newNode.getParentKey(),
			...
			if (strucNode.hasRightSibling()) {
				final StructNode rightSiblingNode = pageTrx.prepareRecordForModification(strucNode.getRightSiblingKey(),
						IndexType.PATH_SUMMARY, 0);
			...
			if (strucNode.hasLeftSibling()) {
				final StructNode leftSiblingNode = pageTrx.prepareRecordForModification(strucNode.getLeftSiblingKey(),
						IndexType.PATH_SUMMARY, 0);
	...
	public void adaptPathForChangedNode(final ImmutableNameNode node, final QNm name, final int uriKey,
		...
		if (type == OPType.SETNAME && pathSummaryReader.getReferences() == 1) {
			...
			if (axis.hasNext()) {
				...
				// Found node.
				processFoundPathNode(oldPathNodeKey, pathSummaryReader.getNodeKey(), node.getNodeKey(), name, uriKey,
						prefixKey, localNameKey);
			} else {
				if (pathSummaryReader.getKind() == NodeKind.XML_DOCUMENT
					...
					final PathNode pathNode = pageTrx.prepareRecordForModification(pathSummaryReader.getNodeKey(),
							IndexType.PATH_SUMMARY, 0);
	...
	private void processFoundPathNode(final @NonNegative long oldPathNodeKey, final @NonNegative long newPathNodeKey,
		...
		final PathNode currNode = pageTrx.prepareRecordForModification(pathSummaryReader.getNodeKey(),
				IndexType.PATH_SUMMARY, 0);
		...
		if (movedNodeCursorToFirstChild && movedPathSummaryToFirstChild) {
			...
			for (final LevelOrderAxis levelOrderAxis = new LevelOrderAxis.Builder(nodeRtx).includeSelf()
					.build(); levelOrderAxis.hasNext();) {
	...
	private void adaptPathSummary(int level, long newPathNodeKey) {
		...
		final Axis axis = new FilterAxis<>(
				new LevelOrderAxis.Builder(pathSummaryReader).filterLevel(level).includeSelf().build(),
	...
	private void removePathSummaryNode(final RemoveSubtreePath remove) {
		...
		if (pathSummaryReader.hasLeftSibling()) {
			final StructNode leftSibling = pageTrx.prepareRecordForModification(pathSummaryReader.getLeftSiblingKey(),
					IndexType.PATH_SUMMARY, 0);
		...
		if (pathSummaryReader.hasRightSibling()) {
			final StructNode rightSibling = pageTrx.prepareRecordForModification(pathSummaryReader.getRightSiblingKey(),
					IndexType.PATH_SUMMARY, 0);
		...
		StructNode parent = pageTrx.prepareRecordForModification(pathSummaryReader.getParentKey(),
				IndexType.PATH_SUMMARY, 0);
		if (!pathSummaryReader.hasLeftSibling()) {
		...
		//
		// for (final var descendantAxis = new DescendantAxis(pathSummaryReader);
		...
		// System.out.println("rightSiblingKey: " +
		// pathSummaryReader.getRightSiblingKey());
	...
	/**
	 * Decrements the reference-counter of the node or removes the path node if the
	 * reference-counter would be zero otherwise.
	 *
	 * @param node
	 *            node which is going to be removed from the storage
	 * @throws SirixException
	 *             if anything went wrong
	 */
