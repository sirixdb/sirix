...
public final class Names {
	 ...
	 *
	 * @param indexNumber
	 *            the index number / offset of the names instance
	 */
	 ...
	 * @param indexNumber
	 *            the kind of name dictionary
	 * @param maxNodeKey
	 *            the maximum node key
	 */
	private Names(final PageReadOnlyTrx pageReadTrx, final int indexNumber, final long maxNodeKey) {
		...
		// TODO: Next refactoring iteration: Move this to a factory, just assign stuff
		// in constructors
	...
	public void removeName(final int key, final PageTrx pageTrx) {
		...
		if (prevValue != 0) {
			...
			if (prevValue - 1 == 0) {
				...
				final HashCountEntryNode hashCountEntryNode = pageTrx.prepareRecordForModification(countNodeKey,
						IndexType.NAME, indexNumber);
	...
	public int setName(final String name, final PageTrx pageTrx) {
		...
		if (previousStringValue == null || !previousStringValue.equals(name)) {
			...
			final HashCountEntryNode hashCountEntryNode = pageTrx.prepareRecordForModification(nodeKey, IndexType.NAME,
					indexNumber);
	 ...
	 * @return cloned index
	 */
	public static Names fromStorage(final PageReadOnlyTrx readOnlyPageTrx, final int indexNumber,
