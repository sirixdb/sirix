...
public interface ChangeListener {
  ...
  /**
   * Commit-time lifecycle hook, called once per commit by the owning {@link IndexController} after
   * the pre-commit hooks and BEFORE page serialization, so index writes issued here still ride the
   ...
   *
   * <p>
   * The distinction only matters to a listener that builds an index ACROSS commits: a load-time
   ...
   *
   * <p>
   * Ordinary entry-at-a-time listeners keep no state beyond the transaction's page log and use this
   * no-op. A listener whose work deliberately spans successful intermediate commits must retire that
   * external state here: rollback, revert and a clean close all sever the lineage that made the
   * accumulated state valid. This hook is intentionally separate from listener rebinding because a
   * successful intermediate commit also rebinds listeners and must preserve that state.
   * </p>
   */
