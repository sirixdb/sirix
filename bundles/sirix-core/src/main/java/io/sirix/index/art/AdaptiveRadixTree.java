...
/**
 * An Adaptive Radix trie based {@link NavigableMap} implementation. The map is
 * sorted according to the {@linkplain BinaryComparable} provided at map
 * creation time.
 *
 * <p>
 * This implementation provides log(k) time cost for the {@code containsKey},
 * {@code get}, {@code put} and {@code remove} operations where k is the length
 * of the key. Algorithms are adaptations of those as described in the
 * <a href="https://db.in.tum.de/~leis/papers/ART.pdf">paper</a> <em>"The
 * Adaptive Radix Tree: ARTful Indexing for Main-Memory Databases"</em> by Dr.
 * Viktor Leis.
 *
 * <p>
 ...
 * iterator is created, in any way except through the iterators own
...
public class AdaptiveRadixTree<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V> {
	...
	/**
	 * The number of structural modifications to the tree. To be touched where ever
	 * size changes.
	 */
	...
	/*
	 * given node only has one child and has a parent. we eliminate this node and
	 * pull up it's only child, linking it with the parent.
	 *
	 ...
	 *
	 * also update child's compressed path updated to: this node's compressed path +
	 * partialKey to child + child's own compressed path
	 */
	...
	/*
	 * updates given node's only child's compressed path to: given node's compressed
	 * path + partialKey to child + child's own compressed path
	 */
	...
	private V put(byte[] keyBytes, K key, V value) {
		...
		while (true) {
			if (node instanceof LeafNode) {
				...
				// we have to replace the prevDepth's child pointer to this new node
	...
	/*
	 * we reached a lazy expanded leaf node, we have to expand it now. but how much
	 * should we expand? since we reached depth X, it means till now both leaf node
	 * and new node have same bytes. now what has been stored lazily is leaf node's
	 * key(depth, end). that's the part over which we need to compute the longest
	 * common prefix. that's the part we can path compress.
	 */
	private static <K, V> Node lazyExpansion(LeafNode<K, V> leaf, byte[] keyBytes, K key, V value, int depth) {
		...
		if (depth == keyBytes.length && depth == leafKey.length) {
			// we're referring to a key that already exists, replace value and return
			// current
			return leaf;
		}
	...
	static void removePessimisticLCPFromCompressedPath(InnerNode node, int depth, int lcp) {
		// lcp cannot be equal to Math.min(InnerNode.PESSIMISTIC_PATH_COMPRESSION_LIMIT,
		// node.prefixLen)
		// it has to be less, else it'd mean the compressed path matches completely
	 ...
	 *
	 * case 1: key has nothing left (can't happen, else they'd be prefixes and our
	 * key transformations must ensure that it is not possible) case 2: prefixLen <=
	 * InnerNode.PESSIMISTIC_PATH_COMPRESSION_LIMIT we're done here, we can do a
	 * findChild for next partial key (caller's depth + lcp + 1) case 3: prefixLen
	 * is more i.e. an optimistic path is left to match. traverse down and get leaf
	 * to match remaining optimistic prefix path. case 3a: optimistic path matches,
	 * we can do findChild for next partial key case 3b: have to split
	 *
	...
	private int matchCompressedPath(InnerNode node, byte[] keyBytes, K key, V value, int depth, InnerNode prevDepth) {
		int lcp = 0;
		int end = Math.min(keyBytes.length - depth,
				Math.min(node.prefixLen, InnerNode.PESSIMISTIC_PATH_COMPRESSION_LIMIT));
		...
		if (lcp == InnerNode.PESSIMISTIC_PATH_COMPRESSION_LIMIT) {
			...
			/*
			 * match remaining optimistic path if we match entirely we return with new depth
			 * and caller can proceed with findChild (depth + lcp + 1) if we don't match
			 * entirely, then we split
			 */
	...
	// called when lcp has become more than
	// InnerNode.PESSIMISTIC_PATH_COMPRESSION_LIMIT
	...
	/*
	 * On level X match compressed path of "this" node if matches, then take
	 * follow-on pointer and continue matching if it doesn't, see if compressed path
	 * greater/smaller than key if greater, return the first node of the level i.e.
	 * call first on this node and return. if lesser, go one level up (using parent
	 * link) and find the next partialKey greater than the uplinking partialKey on
	 * level X-1. if you got one, simply take the first child nodes at each down
	 * level and return the leaf (left most traversal) if not, then we got to go on
	 * level X-2 and find the next greater and keep going level ups until we either
	 * find a next greater partialKey or we find root (which will have parent null
	 * and hence search ends).
	 *
	 * What if all compressed paths matched, then when taking the next follow-on
	 * pointer, we reach a leafNode? or a null? if leafNode then it means, until now
	 * the leafNode has the same prefix as the provided key. if leafNode >= given
	 * key, then return leafNode if leafNode < given key, then take leafNode's
	 * parent uplink and find next greater partialKey than the uplinking partialKey
	 * on level leaf-1. if you reach a null, then it means key doesn't exist, but
	 * before taking this previous partialKey, the entire path did exist. Hence, we
	 * come up a level from where we got the null. Find the next higher partialKey
	 * than which we took for null (no uplink from the null node, so we do it before
	...
	private LeafNode<K, V> getHigherOrCeilEntry(boolean ceil, byte[] key) {
		...
		while (true) {
			...
			depth += innerNode.prefixLen;
			if (depth == key.length) {
				// if ceil is true, then we are allowed to return the prefix ending here (leaf
				// of this node)
				// if ceil is false, then we need something higher and not the prefix, hence we
				// start traversal
				// from first()
	 ...
	 *             if the Entry is null Note: taken from TreeMap
	 */
	 ...
	 * copes with {@code null} o1 properly. Note: Taken from TreeMap
	 */
