 ...
 *
 * <p>
 * Both are the paper's cheapest schemes and both hit distributions we actually store. A projection
 * column of per-row set CARDINALITIES is the clearest case: nearly every record carries the same
 * number of elements, so the column is one value with a handful of exceptions, and bit-packing
 * still spends its width on every row. Sorted or clustered scalar columns — a year beside records
 * inserted in year order — are RLE's case.
 *
 * <p>
 * Neither is a general win. RLE stores a length beside every value, so on data without runs it is
 * strictly LARGER than the input, which is why BtrBlocks excludes it from estimation when the
 ...
 *
 * <p>
 * BtrBlocks optimises DECOMPRESSION; our fast paths try never to decompress at all, evaluating
 * predicates over the encoded bytes. The two aims agree for One Value and disagree for RLE:
 *
 * <ul>
 * <li>One Value makes scanning strictly cheaper — one comparison decides the whole block, which is
 * a zone map that happens to also be the data.</li>
 * <li>RLE makes a positional scan HARDER: the run at index {@code i} is only known by walking the
 * lengths. A predicate can be evaluated per RUN rather than per value, which is a large win when
 * runs are long, but it is a different kernel, not the existing one.</li>
 * </ul>
 *
 * <p>
 * So the ratio these buy is not automatically speed, and the selector must not be given RLE for a
 * column whose scan has no run-aware kernel. Recorded here because the paper's headline numbers are
 * decompression-bound and ours are scan-bound, and that difference decides which of the two schemes
 * is worth taking.
 */
public final class LightweightSchemes {
   ...
   *
   * <p>
   * Sized the way the encoder writes it rather than as {@code runs * 16}: both arrays are packed to
   * the width their own maximum needs, and on the columns RLE is chosen for — few distinct values,
   * long runs — that is several times smaller than a byte-aligned estimate. Estimating high here
   * would make the selector prefer bit-packing on data where RLE genuinely wins, which is the failure
   * mode the paper's sample-and-measure approach exists to avoid.
   */
   ...
   */
  public static int decodeRle(final long[] runValues, final int[] runLengths, final int runs, final long[] out) {
  ...
  static int widthOf(final long maxValue) {
    return maxValue <= 0
        ? 0
        : 64 - Long.numberOfLeadingZeros(maxValue);
  }
