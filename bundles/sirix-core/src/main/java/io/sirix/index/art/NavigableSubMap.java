...
abstract class NavigableSubMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V> {
   ...
   * If fromStart is true, then the low (absolute) bound is the start of the backing map, and the
   * other values are ignored. Otherwise, if loInclusive is true, lo is the inclusive bound, else lo
  ...
  NavigableSubMap(AdaptiveRadixTree<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi,
    ...
    this.loBytes = fromStart
        ? null
        : m.binaryComparable().get(lo);
    this.hiBytes = toEnd
        ? null
        : m.binaryComparable().get(hi);
    if (!fromStart && !toEnd) {
  ...
  final boolean inClosedRange(K key) {
    ...
    // if we have a lower bound, then this key ought to be higher than our lower bound (closed, hence
    // including).
    // if we have an upper bound, then this key ought to be lower than our upper bound (closed, hence
    // including).
  ...
  /*
   * Absolute versions of relation operations. Subclasses map to these using like-named "sub" versions
   * that invert senses for descending maps
   */
  ...
  final LeafNode<K, V> absLowest() {
    LeafNode<K, V> e = (fromStart
        ? tree.getFirstEntry()
        : (loInclusive
            ? tree.getCeilingEntry(loBytes)
            : tree.getHigherEntry(loBytes)));
    return (e == null || tooHigh(e.getKey()))
        ? null
        : e;
  }
  ...
  final LeafNode<K, V> absHighest() {
    LeafNode<K, V> e = (toEnd
        ? tree.getLastEntry()
        : (hiInclusive
            ? tree.getFloorEntry(hiBytes)
            : tree.getLowerEntry(hiBytes)));
    return (e == null || tooLow(e.getKey()))
        ? null
        : e;
  }
  ...
  /*
   * Iterators for SubMaps that understand the submap's upper and lower bound while iterating. Fence
   * is one of the bounds depending on the kind of iterator (ascending, descending) and first becomes
   * the other one to start from.
   */
