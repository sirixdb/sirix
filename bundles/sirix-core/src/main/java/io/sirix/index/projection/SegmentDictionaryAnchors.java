 ...
 * at page-encode time the segment's dictionary has not been written and has no storage key yet. This
 * is the table that closes the gap: the writer fills it as each segment is persisted, and the read
 * side translates a page's segment anchor into the header key {@code GlobalValueDictionary} resolves
 * against.
 * </p>
 *
 * <h2>Why the sealed count is stored beside the key</h2>
 *
 * A page also records the entry count it saw. The reader's validity rule is that the dictionary must
 * hold at least that many entries — a live count BELOW a recorded one means the key was reused by
 * something else, and ids resolved against it would be plausible and wrong. A segment dictionary is
 * sealed once and never appended to afterwards, so its sealed count is the whole answer and no
 ...
 * and read concurrently, and a segment absent from it is simply one whose pages must keep their bytes.
 * </p>
 *
  ...
  public void seal(final long segment, final int column, final long headerKey, final int sealedEntryCount) {
    ...
    if (previous != null && !previous.equals(sealed)) {
      throw new IllegalStateException(
          "segment " + segment + " column " + column + " was already sealed at header key " + previous.headerKey()
              + " with " + previous.sealedEntryCount() + " entries; refusing to reseal at " + headerKey + " with "
              + sealedEntryCount);
    }
  }
