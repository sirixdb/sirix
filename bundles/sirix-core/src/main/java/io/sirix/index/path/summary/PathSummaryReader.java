...
public final class PathSummaryReader implements NodeReadOnlyTrx, NodeCursor {
	...
	private PathSummaryReader(final PageReadOnlyTrx pageReadTrx,
		...
		if (pathSummaryData == null || pageReadTrx.hasTrxIntentLog()) {
			currentNode = this.pageReadTrx.getRecord(Fixed.DOCUMENT_NODE_KEY.getStandardProperty(),
			...
			final int maxNrOfNodes = (int) this.pageReadTrx
					.getPathSummaryPage(this.pageReadTrx.getActualRevisionRootPage()).getMaxNodeKey(0);
	 ...
	 *
	 * @param pageReadTrx
	 *            the {@link PageReadOnlyTrx} instance
	 * @param resourceSession
	 *            the {@link ResourceSession} instance
	...
	void putMapping(final @NonNegative long pathNodeKey, final StructNode node) {
		if (pathNodeKey >= pathNodeMapping.length) {
			...
			pathNodeMapping = Arrays.copyOf(pathNodeMapping,
					pathNodeKey >= nodeCountTimes2 ? (int) pathNodeKey * 2 : nodeCountTimes2);
		}
	...
	/**
	 * Match all descendants of the node denoted by its {@code pathNodeKey} with the
	 * given {@code name}.
	 *
	 * @param name
	 *            the QName
	 ...
	 * @param includeSelf
	 *            if current node should be included or not
