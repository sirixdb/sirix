...
/**
 * Simple Red-black tree reader (balanced binary search-tree -- based on
 * BaseX(.org) version).
 *
 * @param <K>
...
public final class RBTreeReader<K extends Comparable<? super K>, V extends References> implements NodeCursor {
	 ...
	 *
	 * @param <K>
	 *            key instance which extends comparable
	 * @param <V>
	 *            value
	 * @param cache
	 *            a cache shared between all read-only tree readers
	 * @param pageReadTrx
	 *            {@link PageReadOnlyTrx} for persistent storage
	 * @param type
	 *            type of index
	 ...
	 * @param cache
	 *            a cache shared between all read-only tree readers
	 * @param pageReadOnlyTrx
	 *            {@link PageReadOnlyTrx} for persistent storage
	 * @param indexType
	 *            kind of indexType
	 * @param indexNumber
	 *            the indexNumber number
	 */
	private RBTreeReader(final Cache<RBIndexKey, Node> cache, final PageReadOnlyTrx pageReadOnlyTrx,
		...
		currentNode = this.pageReadOnlyTrx.getRecord(Fixed.DOCUMENT_NODE_KEY.getStandardProperty(), indexType,
				indexNumber);
		...
		if (!(pageReadOnlyTrx instanceof PageTrx)) {
			for (RBNodeIterator it = new RBNodeIterator(0); it.hasNext();) {
				...
				this.cache.put(new RBIndexKey(node.getNodeKey(), revisionNumber, indexType, indexNumber),
						getCurrentNodeAsRBNodeKey());
			}
	 ...
	 *
	 * @param startNodeKey
	 *            the key of the node to start from
	 * @param key
	 *            key to be found
	 * @param mode
	 *            the search mode
			...
			if (c < 0) {
				if (node.getLeftChild() != null) {
					...
					node = pageReadOnlyTrx instanceof PageTrx
							? null
							: (RBNodeKey<K>) cache.get(
									new RBIndexKey(node.getLeftChildKey(), revisionNumber, indexType, indexNumber));
				...
				if (node.getRightChild() != null) {
					...
					node = pageReadOnlyTrx instanceof PageTrx
							? null
							: (RBNodeKey<K>) cache.get(
									new RBIndexKey(node.getRightChildKey(), revisionNumber, indexType, indexNumber));
	...
	/**
	 * Finds the specified key in the index starting from the specified node key and
	 * returns its AVLNode.
	 *
	 * @param startNodeKey
	 *            specified node
	 ...
	 */
	public Optional<RBNodeKey<K>> getCurrentNodeAsRBNodeKey(final long startNodeKey, final K key,
	 ...
	 *            the search mode
	 * @param comp
	 *            comparator to be used to compare keys
	 * @return Optional {@link RBNodeKey} reference
	 */
	public Optional<RBNodeKey<K>> getCurrentNodeAsRBNodeKey(final K key, final SearchMode mode,
