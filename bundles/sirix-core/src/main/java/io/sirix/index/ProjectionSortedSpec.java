 ...
 *
 * <p>
 * Column numbers refer to the projection definition's ordered field list. Every projected record is
 * kept in the view, ordered by the key columns and then by its stable record key. The declaration
 * names columns only: a query's equality filter on a leading prefix of the key columns is answered
 * as a key range at query time. This declaration is persisted with the index catalogue and applies
 * identically to initial construction and fine-grained maintenance.
 * </p>
 */
