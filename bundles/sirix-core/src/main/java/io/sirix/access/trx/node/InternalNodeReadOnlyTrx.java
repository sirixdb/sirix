...
public interface InternalNodeReadOnlyTrx<N extends ImmutableNode> extends NodeCursor, NodeReadOnlyTrx {
  ...
  /**
   * Returns a live view of the current structural node without allocating a snapshot. When the cursor
   * is in singleton mode, returns the singleton directly (zero-alloc). The returned reference must
   * NOT be retained across moveTo/prepareRecordForModification calls — extract needed values into
   * local primitives immediately.
   */
  ...
  /**
   * Apply the logical cursor side effects of {@link #moveTo(long)} after a caller has proved that the
   * current physical node binding can be reused.
   *
   * <p>
   * This is an internal, narrowly scoped hook. It does not resolve or rebind a node and must only be
   * used for an approved physical self-move shortcut.
   * </p>
   */
   ...
   *
   * <p>
   * The implementation rejects writer-identity, key, page, and slot-address mismatches before
   ...
   * create/link/position sequence of an insertion.
   * </p>
   *
