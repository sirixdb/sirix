...
public abstract class AbstractNodeTrxImpl<R extends NodeReadOnlyTrx & NodeCursor, W extends NodeTrx & NodeCursor, NF extends NodeFactory, N extends ImmutableNode, IN extends InternalNodeReadOnlyTrx<N>>
	...
	/**
	 * Collects update operations in pre-order, thus it must be an order-preserving
	 * sorted map.
	 */
	protected final SortedMap<SirixDeweyID, DiffTuple> updateOperationsOrdered;

	/**
	 * Collects update operations in no particular order (if DeweyIDs used for
	 * sorting are not stored).
	 */
	...
	/**
	 * The {@link IndexController} used within the resource manager this
	 * {@link NodeTrx} is bound to.
	 */
	...
	private void reInstantiate(final @NonNegative long trxID, final @NonNegative int revNumber) {
		...
		pageTrx = resourceSession.createPageTransaction(trxID, revNumber, revNumber, InternalResourceSession.Abort.NO,
				true);
	...
	public synchronized W rollback() {
		...
		pageTrx = resourceSession.createPageTransaction(trxID, revNumber, revNumber, InternalResourceSession.Abort.YES,
				true);
		nodeReadOnlyTrx.setPageReadTransaction(pageTrx);
		resourceSession.setNodePageWriteTransaction(getId(), pageTrx);
	...
	public W revertTo(final int revision) {
		...
		try {
			...
			resourceSession.closeNodePageWriteTransaction(getId());
			pageTrx = resourceSession.createPageTransaction(trxID, revision, revNumber - 1,
					InternalResourceSession.Abort.NO, true);
			nodeReadOnlyTrx.setPageReadTransaction(null);
			nodeReadOnlyTrx.setPageReadTransaction(pageTrx);
			resourceSession.setNodePageWriteTransaction(getId(), pageTrx);
	...
	public String toString() {
		return MoreObjects.toStringHelper(this).add("super", super.toString()).add("hashType", this.hashType)
				.add("nodeReadOnlyTrx", this.nodeReadOnlyTrx).toString();
	}
}
