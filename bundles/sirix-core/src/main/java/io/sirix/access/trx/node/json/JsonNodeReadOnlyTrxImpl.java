...
public final class JsonNodeReadOnlyTrxImpl
		extends
			AbstractNodeReadOnlyTrx<JsonNodeReadOnlyTrx, JsonNodeTrx, ImmutableNode>
		implements
			InternalJsonNodeReadOnlyTrx {

	/**
	 * Constructor.
	 *
	 * @param resourceManager
	 *            the current {@link ResourceSession} the reader is bound to
	 * @param trxId
	 *            ID of the reader
	 * @param pageReadTransaction
	 *            {@link PageReadOnlyTrx} to interact with the page layer
	 * @param documentNode
	 *            the document node
	 */
	JsonNodeReadOnlyTrxImpl(final InternalResourceSession<JsonNodeReadOnlyTrx, JsonNodeTrx> resourceManager,
			final @NonNegative long trxId, final PageReadOnlyTrx pageReadTransaction,
	...
	public List<JsonObject> getUpdateOperations() {
		...
		final var updateOperationsFile = resourceSession.getResourceConfig().getResource()
	...
	private Consumer<JsonElement> serializeJsonFragmentIfNeeded(final List<JsonObject> diffTuples) {
		return diff -> {
			...
			if (diffTupleObject != null
					&& "jsonFragment".equals(diffTupleObject.getAsJsonPrimitive("type").getAsString())) {
	...
	public List<JsonObject> getUpdateOperationsInSubtreeOfNode(final SirixDeweyID rootDeweyId, final long maxDepth) {
		...
		return updateOperations.stream().filter(filterAncestorOperations(rootDeweyId, maxDepth)).sorted(sortByDeweyID())
	...
	public boolean isBooleanValue() {
		assertNotClosed();
		final var currentNode = getCurrentNode();
		return currentNode.getKind() == NodeKind.BOOLEAN_VALUE
