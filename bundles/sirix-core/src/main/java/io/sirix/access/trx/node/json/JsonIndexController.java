...
public final class JsonIndexController extends AbstractIndexController<JsonNodeReadOnlyTrx, JsonNodeTrx> {
		...
		for (final IndexDef indexDef : indexDefs) {
			switch (indexDef.getType()) {
				case PATH -> indexBuilders.add(
						createPathIndexBuilder(nodeWriteTrx.getPageWtx(), nodeWriteTrx.getPathSummary(), indexDef));
				case CAS -> indexBuilders.add(createCASIndexBuilder(nodeWriteTrx, nodeWriteTrx.getPageWtx(),
	...
	private JsonNodeVisitor createPathIndexBuilder(final PageTrx pageWriteTrx,
