...
public interface IndexController<R extends NodeReadOnlyTrx & NodeCursor, W extends NodeTrx & NodeCursor> {
	 ...
	 * @return {@code true} if an index of the specified type exists, {@code false}
	 *         otherwise
	 */
	 ...
	 *            the {@link ResourceSession} this index controller is bound to
	 * @param revision
	 *            the revision for this index controller
	 * @return {@code true} if an index of the specified type exists, {@code false}
	 *         otherwise
	 ...
	 */
	static boolean containsIndex(final IndexType type, final ResourceSession<?, ?> resourceSession,
			final int revision) {
		final Indexes indexes = new Indexes();

		final java.nio.file.Path indexesFile = resourceSession.getResourcePath()
