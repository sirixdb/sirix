 ...
 *
 * <p>The copy/decode completes synchronously while the cursor is bound. No page-backed segment,
 * slice, or array escapes through this API. Callers that receive {@link #UNAVAILABLE} must preserve
 * behavior by falling back to the public value API.</p>
 */
public interface FusedStringCursor {
   ...
   * zero.
   *
   * <p>A non-negative result is the number of bytes written. A result less than
   * {@link #UNAVAILABLE} means the destination is too small; {@link #requiredCapacity(int)} returns
   * the capacity with which the caller must retry. The destination is left untouched in both
   * negative-result cases.</p>
   *
