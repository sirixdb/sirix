...
/**
 * A database session factory for XML databases. Replaces the Dagger-managed
 * {@code XmlLocalDatabaseComponent} and {@code XmlLocalDatabaseModule} with direct
 * constructor-based wiring.
 *
 * @author Joao Sousa
 */
public final class LocalXmlDatabaseFactory implements LocalDatabaseFactory<XmlResourceSession> {
  ...
  public Database<XmlResourceSession> createDatabase(final DatabaseConfiguration configuration, final User user) {
    ...
    // Database-scoped dependencies (previously in LocalDatabaseModule + XmlLocalDatabaseModule)
