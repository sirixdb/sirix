...
public interface NodeReadOnlyTrx extends AutoCloseable {
	 ...
	 *             if can't close {@link XmlNodeReadOnlyTrx}
	 */
	@Override
	void close();
	 ...
	 *
	 * @param key
	 *            the nodeKey of the node to move to
	...
	/**
	 * Get the kind of path of the current node (only supported for path summary
	 * transactions).
	 *
	 ...
	 *
	 * @return {@code true}, if the current node is the document root node,
	 *         {@code false} otherwise
	 */
	boolean isDocumentRoot();

	/**
	 * Determines if the state of the transaction has been changed to
	 * {@code closed}.
	 *
	 * @return {@code true}, if the transaction has been closed, {@code false}
	 *         otherwise
	 */
	boolean isClosed();
	 ...
	 *
	 * @return {@code true}, if the current node has children, {@code false}
	 *         otherwise
	 */
	boolean hasChildren();
	 ...
	 *
	 * @return the value of the current node or {@code null} if the node has no
	 *         value
	 */
	String getValue();
	...
	/**
	 * Get the revision number of the previous revision of the node, the
	 * transactional cursor currently points to.
	 *
