...
public interface PageTrx extends PageReadOnlyTrx {
	...
	/**
	 * Create fresh key/record (record must be a record) and prepare
	 * key/record-tuple for modifications (CoW). The record might be a node, in this
	 * case the key is the node.
	 *
	 * @param record
	 *            record to add (usually a node)
	 * @param indexType
	 *            the index type
	 * @param index
	 *            the index number
	 ...
	 * for upcoming modification. The key of the entry might be the node key and the
	 ...
	 */
	<V extends DataRecord> V prepareRecordForModification(@NonNegative long key, @NonNull IndexType indexType,
			int index);
	 ...
	 *             if {@code recordKey < 0}
	 * @throws NullPointerException
	 *             if {@code indexType} is {@code null}
	 */
	...
	/**
	 * Commit the transaction, that is persist changes if any and create a new
	 * revision. The commit message is going to be persisted as well.
	 *
	 * @param commitMessage
	 *            the commit message
	...
	/**
	 * Get the revision, which this page trx is going to represent in case of a
	 * revert.
	 *
