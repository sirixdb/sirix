...
public final class ModificationVisitor extends AbstractXmlNodeVisitor {
	...
	/**
	 * Determines if a node must be modified. If yes, it is deleted and {@code true}
	...
	private VisitResult delete() throws SirixException {
		try {
			...
			boolean removeTextNode = wtx.getLeftSiblingKind() == NodeKind.TEXT
					&& wtx.getRightSiblingKind() == NodeKind.TEXT;
			wtx.moveTo(nodeKey);
