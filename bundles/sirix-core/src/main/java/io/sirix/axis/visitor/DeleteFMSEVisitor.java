...
public class DeleteFMSEVisitor extends AbstractXmlNodeVisitor {
	...
	/**
	 * Determines if a node must be deleted. If yes, it is deleted and {@code true}
	 * is returned. If it must not be deleted {@code false} is returned. The
	 * transaction is moved accordingly in case of a remove-operation such that the
	 * {@link DescendantAxis} can move to the next node after a delete occurred.
	 *
	 * @param node
	...
	private VisitResult delete(final ImmutableNode node) {
		try {
			...
			if (wtx.hasLeftSibling() && wtx.moveToLeftSibling() && wtx.getKind() == NodeKind.TEXT
					&& wtx.moveToRightSibling() && wtx.hasRightSibling() && wtx.moveToRightSibling()
