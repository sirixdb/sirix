...
/**
 * Provide standard Java iterator capability compatible with the enhanced for
 * loop available since Java 5. Override the "template method"
 * {@code nextNode()} to implement an axis. Return {@code done()} if the axis
...
public abstract class AbstractAxis implements Iterator<PathNode>, Iterable<PathNode> {
	 ...
	 *             if {@code nodeCursor} is {@code null}
	 */
	 ...
	 * @throws NullPointerException
	 *             if {@code nodeCursor} or {@code includeSelf} is {@code null}
	 */
	 ...
	 * false. Is callable from subclasses which implement {@link #nextNode()} to
	 ...
	 *
	 * <p>
	 * During the last call to {@code hasNext()}, that is {@code hasNext()} returns
	 * false, the transaction is reset to the start key.
	 * </p>
	 *
	 * <p>
	 ...
	 * either call {@link #done()} when there are no elements left in the iteration
	 * or return the node key {@code EFixed.NULL_NODE.getStandardProperty()}.
	 *
	 * <p>
