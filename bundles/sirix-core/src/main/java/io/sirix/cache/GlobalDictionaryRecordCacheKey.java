 ...
 * carries them: a node key is unique only within a resource. {@code revision} is present because the
 * dictionary sub-trie is copy-on-write with freshly minted keys, with ONE exception — the generation
 * header is rewritten under a stable key — so a revision-free key could serve a pre-rewrite header.
 * The write path evicts that key explicitly as well; the revision is the belt to that braces.
 * </p>
 *
