...
public final class RecordPageCache implements Cache<PageReference, KeyValueLeafPage> {
	...
	public RecordPageCache(final int maxSize) {
		final RemovalListener<PageReference, KeyValueLeafPage> removalListener = (PageReference key, KeyValueLeafPage _,
				RemovalCause cause) -> {
			key.setPage(null);
		};

		pageCache = Caffeine.newBuilder().initialCapacity(maxSize).executor(Runnable::run).maximumSize(maxSize)
