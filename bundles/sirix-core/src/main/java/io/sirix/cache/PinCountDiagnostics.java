 ...
 * <p>
 * This utility provides insights into cached pages and their memory usage, helping identify
 * potential memory leaks and cache efficiency issues.
 * <p>
 * <b>Note:</b> Pin counting has been replaced by guard-based page protection. The guard count
 * mechanism (see {@link KeyValueLeafPage#getGuardCount()}) now provides the primary protection
 * against page eviction during active use.
 *
 * @author Johannes Lichtenberger
 * @deprecated This class uses the legacy pin count terminology. Guard counts should be used for
 *             analyzing page protection status.
 */
@Deprecated
public class PinCountDiagnostics {
  ...
  public static class DiagnosticReport {
    ...
    public DiagnosticReport(int totalPages, int pinnedPages, int unpinnedPages, long totalMemoryBytes,
        long pinnedMemoryBytes, long unpinnedMemoryBytes, Map<Integer, Integer> pinCountDistribution,
    ...
    public String toString() {
      ...
      sb.append(String.format("Pinned Pages: %d (%.1f%%)\n", pinnedPages, totalPages > 0
          ? 100.0 * pinnedPages / totalPages
          : 0));
      sb.append(String.format("Unpinned Pages: %d (%.1f%%)\n", unpinnedPages, totalPages > 0
          ? 100.0 * unpinnedPages / totalPages
          : 0));
      ...
      sb.append(String.format("Pinned Memory: %.2f MB (%.1f%%)\n", pinnedMemoryBytes / (1024.0 * 1024.0),
          totalMemoryBytes > 0
              ? 100.0 * pinnedMemoryBytes / totalMemoryBytes
              : 0));
      sb.append(String.format("Unpinned Memory: %.2f MB (%.1f%%)\n", unpinnedMemoryBytes / (1024.0 * 1024.0),
          totalMemoryBytes > 0
              ? 100.0 * unpinnedMemoryBytes / totalMemoryBytes
              : 0));

      sb.append("\nPin Count Distribution:\n");
      pinCountDistribution.entrySet()
                          .stream()
                          .sorted(Map.Entry.comparingByKey())
                          .forEach(
                              e -> sb.append(String.format("  Pin Count %d: %d pages\n", e.getKey(), e.getValue())));
      ...
      if (!pagesWithPins.isEmpty()) {
        ...
        pagesWithPins.stream()
                     ...
                     .forEach(p -> sb.append(String.format(
                         "  Page %d (rev %d, type %s): pinCount=%d, memory=%.2f KB, transactions=%s\n", p.pageKey,
                         p.revision, p.indexType, p.totalPinCount, p.memoryBytes / 1024.0, p.pinnedByTransactions)));
      }
  ...
  public static class PagePinInfo {
    ...
    public PagePinInfo(long pageKey, int revision, String indexType, int totalPinCount, long memoryBytes,
        Map<Integer, Integer> pinnedByTransactions) {
  ...
  public static DiagnosticReport scanCache(Cache<PageReference, ? extends Page> cache) {
    ...
    return new DiagnosticReport(totalPages, 0, totalPages, totalMemoryBytes, 0, totalMemoryBytes, new HashMap<>(),
        new ArrayList<>());
  }
  ...
  public static DiagnosticReport scanBufferManager(BufferManager bufferManager) {
    ...
    fragmentReport.pinCountDistribution.forEach((k, v) -> combinedDistribution.merge(k, v, Integer::sum));
    pageReport.pinCountDistribution.forEach((k, v) -> combinedDistribution.merge(k, v, Integer::sum));
    ...
    return new DiagnosticReport(recordPageReport.totalPages + fragmentReport.totalPages + pageReport.totalPages,
        ...
        combinedDistribution, combinedPins);
  }
  ...
  public static void warnOnTransactionClose(BufferManager bufferManager, int trxId) {
    ...
    if (!leakedPins.isEmpty()) {
      ...
      warning.append("\n⚠️  WARNING: Transaction ")
             .append(trxId)
             .append(" closing with ")
             .append(leakedPins.size())
             .append(" pinned pages!\n");
      ...
      leakedPins.stream()
                ...
                .forEach(p -> warning.append(String.format("    - Page %d (rev %d, type %s): pinCount=%d\n", p.pageKey,
                    p.revision, p.indexType, p.totalPinCount)));
