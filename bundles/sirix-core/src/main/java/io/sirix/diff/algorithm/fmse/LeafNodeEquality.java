...
final class LeafNodeComparator implements NodeComparator<Long> {
	 ...
	 * </ul>
	 * where FMESF is in the range [0,1] and compare() computes the cost of updating
	 * a leaf node.
	 */
	...
	public boolean isEqual(final Long firstNode, final Long secondNode) {
		...
		if (oldRtx.getKind() == NodeKind.ATTRIBUTE || oldRtx.getKind() == NodeKind.NAMESPACE
			...
			if (ratio > FMESF) {
				...
				ratio = nodeComparisonUtils.calculateRatio(
						nodeComparisonUtils.getNodeValue(oldRtx.getNodeKey(), oldRtx),
	...
	private double adaptRatioByUsingThePathChecks(final Path paths) {
		...
		if (paths != Path.PATH_LENGTH_IS_NOT_EQUAL && mId != null) {
			ratio = nodeComparisonUtils.checkIfAncestorIdsMatch(oldRtx.getNodeKey(), newRtx.getNodeKey(), mId) ? 1 : 0;
		} else if (paths == Path.MATCHES) {
			ratio = 1;
		} else if (paths == Path.NO_PATH) {
			ratio = nodeComparisonUtils.checkAncestors(oldRtx.getNodeKey(), newRtx.getNodeKey()) ? 1 : 0;
		} else {
			ratio = 0;
		}
		return ratio;
