...
public final class JsonRecordSerializer implements Callable<Void> {
	...
	/**
	 * Initialize XMLStreamReader implementation with transaction. The cursor points
	 * to the node the XMLStreamReader starts to read.
	 *
	 * @param resourceMgr
	 *            resource manager to read the resource
	 * @param builder
	 ...
	 *            number of records to serialize
	 * @param writer
	...
	public static final class Builder {
		...
		/**
		 * Determines if SirixDB meta data should be serialized for JSON object key
		 * nodes or not.
		 */
		private boolean withMetaData;

		/**
		 * Determines the maximum level to up to which to skip subtrees from
		 * serialization.
		 */
		 ...
		 *            revisions to serialize
		 */
		public Builder(final JsonResourceSession resourceMgr, final int numberOfRecords,
				final @NonNegative long nodeKey, final Writer stream, final JsonSerializerProperties properties,
				final int... revisions) {
		 ...
		 *
		 * @param maxLevel
		 *            the maximum level until which to serialize
		 * @return this reference
		 */
	...
	/**
	 * Serialize the first {@code numberOfRecords}, that is the first n-nodes of the
	 * 1st level.
	 */
	public Void call() {
		...
		for (int i = 1; i <= length; i++) {
			try (final JsonNodeReadOnlyTrx rtx = resourceMgr
				...
				if (rtx.hasFirstChild()) {
					...
					var jsonSerializer = new JsonSerializer.Builder(rtx.getResourceSession(), out, revisions)
							.startNodeKey(rtx.getNodeKey()).serializeStartNodeWithBrackets(false)
							.serializeTimestamp(serializeTimestamp).withMetaData(withMetaData)
							.withNodeKeyAndChildCountMetaData(withNodeKeyAndChildNodeKeyMetaData)
							.withNodeKeyMetaData(withNodeKeyMetaData).numberOfNodes(numberOfNodes).build();
					...
					if (rtx.hasFirstChild()) {
						...
						if (hasMoved) {
							...
							jsonSerializer = new JsonSerializer.Builder(rtx.getResourceSession(), out, revisions)
									.startNodeKey(nodeKey).serializeStartNodeWithBrackets(false).maxLevel(maxLevel)
									.maxChildren(maxChildNodes).serializeTimestamp(serializeTimestamp)
									.withMetaData(withMetaData)
									.withNodeKeyAndChildCountMetaData(withNodeKeyAndChildNodeKeyMetaData)
									.withNodeKeyMetaData(withNodeKeyMetaData).numberOfNodes(numberOfNodes).build();
							jsonSerializer.call();
							if (rtx.isObjectKey()
									&& (withMetaData || withNodeKeyAndChildNodeKeyMetaData || withNodeKeyMetaData)) {
								out.append("}");
							}
							rtx.moveTo(nodeKey);
							...
							if (rtx.hasRightSibling()) {
								for (int j = 1; j < numberOfRecords && rtx.hasRightSibling(); j++) {
									...
									jsonSerializer = new JsonSerializer.Builder(rtx.getResourceSession(), out,
											revisions).startNodeKey(nodeKey).serializeStartNodeWithBrackets(false)
											.maxLevel(maxLevel).maxChildren(maxChildNodes)
											.serializeTimestamp(serializeTimestamp).withMetaData(withMetaData)
											.withNodeKeyAndChildCountMetaData(withNodeKeyAndChildNodeKeyMetaData)
											.withNodeKeyMetaData(withNodeKeyMetaData).numberOfNodes(numberOfNodes)
											.build();
									jsonSerializer.call();
									if (rtx.isObjectKey() && (withMetaData || withNodeKeyAndChildNodeKeyMetaData
											|| withNodeKeyMetaData)) {
