...
public final class BasicJsonDiff implements DiffObserver, JsonDiff {
	 ...
	 * @param oldRevisionNumber
	 *            the revision number of the older revision
	 * @param newRevisionNumber
	 *            the revision number of the newer revision
	 * @return a JSON-String describing the differences encountered between the two
	 *         revisions
	 */
	@Override
	...
	public String generateDiff(JsonResourceSession resourceManager, int oldRevisionNumber, int newRevisionNumber,
		...
		return new JsonDiffSerializer(this.databaseName, resourceManager, oldRevisionNumber, newRevisionNumber, diffs)
				.serialize(true);
	}
