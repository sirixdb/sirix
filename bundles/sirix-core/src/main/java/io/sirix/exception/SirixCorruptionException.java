 ...
 * 
 * <p>This exception indicates that the data read from storage does not match
 * the expected checksum, which typically means:
 * <ul>
 ...
 * </p>
 * 
 * <p>In a single-node deployment, corruption cannot be repaired automatically.
 * The application should consider:</p>
 * <ul>
...
public final class SirixCorruptionException extends SirixRuntimeException {
   ...
   * 
   * <p>The constructor logs the error at ERROR level with hex-encoded hashes
   ...
   */
  public SirixCorruptionException(long pageKey, String context, 
                                   byte[] expectedHash, byte[] actualHash) {
    super(buildMessage(pageKey, context, expectedHash, actualHash));
    this.pageKey = pageKey;
    this.context = context;
    this.expectedHash = expectedHash != null ? expectedHash.clone() : null;
    this.actualHash = actualHash != null ? actualHash.clone() : null;
    
    // Log detailed error information
    LOGGER.error("PAGE CORRUPTION DETECTED: key={}, context={}, expected={}, actual={}",
        pageKey, context, 
        PageHasher.toHexString(expectedHash), 
        PageHasher.toHexString(actualHash));
  }
  ...
  private static String buildMessage(long pageKey, String context, 
                                      byte[] expectedHash, byte[] actualHash) {
    return String.format(
        "Page corruption detected at key %d (%s): expected hash %s, actual hash %s",
        pageKey, 
        context,
        PageHasher.toHexString(expectedHash),
        PageHasher.toHexString(actualHash));
  ...
  public byte[] getExpectedHash() {
    return expectedHash != null ? expectedHash.clone() : null;
  }
  ...
  public byte[] getActualHash() {
    return actualHash != null ? actualHash.clone() : null;
  }
}
