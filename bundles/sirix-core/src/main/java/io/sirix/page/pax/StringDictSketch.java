 ...
 *
 * <h2>Why</h2>
 * A string region is the largest thing on a JSON page: on the reference corpus a scan decompresses
 * 1,440 MB of dictionary bytes across the store, which profiling put at 88 % of a
 ...
 *
 * <p>The asymmetry is what makes it worth storing: a negative is <em>exact</em> (no false
 * negatives, ever — every dictionary entry is inserted), and a positive costs only the
 * decompression that would have happened anyway. So the filter can never change an answer, only
 ...
 * </pre>
 *
 * <p>The bits are near-random, so the region table's LZ77 pass will decline to compress them and
 * store the payload raw — which is what we want: the probe reads a few bytes and must not have to
 * decompress anything to do it.
 *
 * <h2>Sizing</h2>
 * {@value #BITS_PER_ENTRY} bits per entry with {@value #HASH_COUNT} probes gives roughly a 1 %
 * false-positive rate. On the reference corpus that is ~440 B per page (~1.6 % added to the store)
 * against 29 KB of dictionary per page that no longer has to be decompressed.
 *
 * <h2>Correctness contract, and why FSST needs no special case</h2>
 * The sketch hashes the bytes as <em>stored</em>, which under FSST means the encoded form. That is
 * not a compromise — it is the point of a lightweight compression scheme: the encoding is a pure
 * function of (value, symbol table), and the writer decides per value whether to store it encoded
 * with exactly the call a reader can repeat ({@code FSSTCompressor.encodeOrNull}). So a probe
 * reproduces the stored form of its literal and asks about that, and the predicate is answered
 * without a byte being decompressed anywhere.
 *
 * <p>A caller that cannot reproduce the encoded form — no symbol table in hand — probes the raw
 * form alone, which can only produce a false <em>positive</em> (it looks for a stored form that may
 * not be the one used), never a false negative, because the caller then goes on to compare the
