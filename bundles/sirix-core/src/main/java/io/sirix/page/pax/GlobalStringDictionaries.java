...
public interface GlobalStringDictionaries {
   ...
   *
   * <p>
   * Asked once per tag per page, never per value, and asked FIRST: it opens the tag's encode run, and
   * a resolver whose mapping can change during a load (one that mints as pages are encoded) fixes the
   * run's answers here. A caller that reaches for {@link #dictionaryKey}, {@link #idOf} or
   * {@link #dictionaryEntryCount} without asking this first may get answers from either side of a
   * change.
   * </p>
   *
   ...
   *
   * <p>
   * Asked once per tag per page, so that a caller can leave a tag holding an over-long value as bytes
   * BEFORE it probes: a resolver that MINTS turns a probe into an entry, and entries minted for a tag
   * that then keeps its bytes are stored for nothing. The default is no limit, which is what a
   * prebuilt dictionary has — it either holds a value or it does not.
   * </p>
   */
