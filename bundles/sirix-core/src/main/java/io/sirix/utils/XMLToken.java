...
public final class XMLToken {
		...
		return ch < 0x80
				...
				: ch < 0x300
						...
						: ch >= 0x370 && ch <= 0x37D || ch >= 0x37F && ch <= 0x1FFF || ch >= 0x200C && ch <= 0x200D
								|| ch >= 0x2070 && ch <= 0x218F || ch >= 0x2C00 && ch <= 0x2EFF
	...
	/**
	 * Returns the codepoint (unicode value) of the specified token, starting at the
	 * specified position. Returns a unicode replacement character for invalid
	 * values.
	 *
	 * @param token
	 *            token
