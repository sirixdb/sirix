...
public class IntToObjectMap<T> implements Iterable<T> {
	...
	private final class IndexIterator implements IntIterator {
		/**
		 * The last used baseHashIndex. Needed for "jumping" from one hash entry to
		 * another.
		 */
		...
		/**
		 * Create the Iterator, make <code>index</code> point to the "first" index which
		 * is not empty. If such does not exist (eg. the map is empty) it would be zero.
		 */
	...
	/**
	 * Implements an Iterator of a generic type T used for iteration over the map's
	 * values.
	 */
	...
	/**
	 * Holds the base hash entries. if the capacity is 2^N, thn the base hash holds
	 * 2^(N+1). It can hold
	 */
	int[] baseHash;

	/**
	 * The current capacity of the map. Always 2^N and never less than 16. We never
	 * use the zero index. It is needed to improve performance and is also used as
	 * "ground".
	 */
	private int capacity;
	/**
	 * All objects are being allocated at map creation. Those objects are "free" or
	 * empty. Whenever a new pair comes along, a pair is being "allocated" or taken
	 * from the free-linked list. as this is just a free list.
	 */
	 ...
	 * hash's with the following next[] and prev[]. Those are also used to store the
	 * "empty" list.
	 */
	int[] next;
	...
	/**
	 * Constructs a map with given capacity. Capacity is adjusted to a native power
	 * of 2, with minimum of 16.
	 *
	 * @param capacity
	 ...
	 * @return true if the object exists in the map (in .equals() meaning). false
	 *         otherwise.
	 */
	 ...
	 * Some methods use the baseHashIndex. If those call find() there's no need to
	...
	/**
	 * Grows the map. Allocates a new map of double the capacity, and fast-insert
	 * the old key-value pairs.
	 */
	@SuppressWarnings("unchecked")
	protected void grow() {
	 ...
	 *            it is big enough; otherwise, use whatever space we have, setting
	 *            the one after the true data as null.
	 *
