...
/**
 * Custom BytesIn interface for reading data from various sources. This interface replaces the
 * Chronicle Bytes dependency and provides a clean abstraction for reading serialized data.
 * 
...
public interface BytesIn<T> {
  ...
  /**
   * Skip forward by the specified number of bytes. This is equivalent to
   * {@code position(position() + bytes)} but may be more efficient.
   * 
