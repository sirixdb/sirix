...
public class MemorySegmentBytesIn implements BytesIn<MemorySegment> {
  ...
  /**
   * Reset this BytesIn to read from a new segment at the given offset. This allows reusing the
  ...
  public void read(byte[] bytes, int offset, int length) {
    ...
    MemorySegment.copy(memorySegment, ValueLayout.JAVA_BYTE, position, bytes, offset, length);
    position += length;
  }
  ...
  public void skip(long bytes) {
    ...
    if (newPosition > memorySegment.byteSize()) {
      throw new IndexOutOfBoundsException(
          String.format("Skip would exceed segment bounds: position=%d, skip=%d, size=%d", position, bytes,
  ...
  public InputStream inputStream() {
    return new InputStream() {
      ...
      public int read(byte[] b, int off, int len) {
        ...
        MemorySegment.copy(memorySegment, ValueLayout.JAVA_BYTE, streamPosition, b, off, toRead);
  ...
  public byte[] toByteArray() {
    ...
    MemorySegment.copy(memorySegment, ValueLayout.JAVA_BYTE, position, result, 0, (int) remainingBytes);
    return result;
  }
}
