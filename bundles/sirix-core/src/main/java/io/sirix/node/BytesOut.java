...
/**
 * Custom BytesOut interface for writing data to various destinations. This interface replaces the
 * Chronicle Bytes dependency and provides a clean abstraction for writing serialized data.
 * 
...
public interface BytesOut<T> extends AutoCloseable {
   ...
   * zero-copy transfer when possible.
   * 
   * <p>
   * <b>Performance note:</b> This default implementation allocates a temporary byte[] as a fallback
   * for implementations without native MemorySegment support. The key implementations
   * ({@link MemorySegmentBytesOut}, {@link PooledBytesOut}) override this method to use direct
   * segment-to-segment copy with zero allocation.
   * 
   ...
   *
   * <p>
   * Default fallback uses {@link #toByteArray()}, which may allocate. Hot-path implementations should
   * override this method to avoid intermediate allocations.
   *
   ...
   */
  default boolean retainsEmptyPipelineIdentityCache() {
    return true;
  }
  ...
  /**
   * Close and release any resources held by this BytesOut. Default implementation does nothing -
   * implementations with resources to release (like off-heap memory) should override this.
   */
  @Override
  default void close() {
