...
/**
 * MemoryLayout-based approach for accessing serialized node data. This replaces the OffsetTable
 * approach with structured memory layouts that provide better type safety and cleaner field access
 * patterns.
 * <p>
 * The layout handles the common structure of serialized nodes: 1. Optional hash field (8 bytes) 2.
 * NodeDelegate fields (variable-length parent offset + 2 ints) 3. StructNodeDelegate fields
 * (variable-length sibling/child offsets + optional counts) 4. Node-specific data (varies by node
 * type)
 */
