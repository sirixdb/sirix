 ...
 *
 * <p>Instead of materializing a DataRecord Java object and re-serializing it, this class reads
 * fixed-width fields at known offsets and writes the compact encoding directly. The output is
 * byte-identical to {@code NodeKind.serialize(sink, FixedSlotRecordMaterializer.materialize(...), config)}.
 *
 * <p>All field offsets are pre-computed as {@code static final} constants from
 * {@link NodeKindLayouts} at class-load time, so the JIT treats them as compile-time constants.
 */
public final class FixedToCompactTransformer {
  ...
  /**
   * Transform a fixed-slot record to compact format, writing the result (including the
   * leading NodeKind id byte) into {@code sink}.
   *
   * <p>The output is byte-identical to
   ...
   */
  public static void transform(final NodeKind nodeKind, final long nodeKey,
      final MemorySegment slot, final ResourceConfiguration config, final BytesOut<?> sink) {
    ...
    switch (nodeKind) {
      ...
      default -> throw new UnsupportedOperationException(
          "FixedToCompactTransformer does not support node kind: " + nodeKind);
    }
  }

  // ════════════════════════════ JSON STRUCTURAL ════════════════════════════

  private static void transformObject(final long nodeKey, final MemorySegment slot,
  ...
  private static void transformArray(final long nodeKey, final MemorySegment slot,
      final ResourceConfiguration config, final BytesOut<?> sink) {
  ...
  private static void transformJsonDocument(final long nodeKey, final MemorySegment slot,
      final BytesOut<?> sink) {
  ...
  private static void transformElement(final long nodeKey, final MemorySegment slot,
      final ResourceConfiguration config, final BytesOut<?> sink) {
    ...
    for (int i = 0; i < attrCount; i++) {
      DeltaVarIntCodec.encodeDelta(sink,
          slot.get(JAVA_LONG_UNALIGNED, attrPointer + (long) i * Long.BYTES), nodeKey);
    }
    ...
    for (int i = 0; i < nsCount; i++) {
      DeltaVarIntCodec.encodeDelta(sink,
          slot.get(JAVA_LONG_UNALIGNED, nsPointer + (long) i * Long.BYTES), nodeKey);
    }
  }

  // ════════════════════════════ XML ATTRIBUTE ════════════════════════════

  private static void transformAttribute(final long nodeKey, final MemorySegment slot,
      final BytesOut<?> sink) {
  ...
  private static void transformNamespace(final long nodeKey, final MemorySegment slot,
      final BytesOut<?> sink) {
  ...
  private static void transformText(final long nodeKey, final MemorySegment slot,
      final BytesOut<?> sink) {
  ...
  private static void transformComment(final long nodeKey, final MemorySegment slot,
      final BytesOut<?> sink) {
