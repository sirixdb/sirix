...
/**
 * High-performance DeweyID encoder optimized for minimal branch mispredictions and direct byte
 * manipulation.
 * 
 * <p>
 * This encoder replaces the bit-by-bit encoding in {@link SirixDeweyID#toBytes()} with lookup
 * tables and direct byte writes for the common tiers (0-3).
 * 
 * <h2>Tier Encoding</h2>
 * 
 * <pre>
...
public final class DeweyIDEncoder {
  ...
  private static final ThreadLocal<byte[]> ENCODE_BUFFER = ThreadLocal.withInitial(() -> new byte[64]);
   ...
   * 
   * <p>
   * This method is functionally equivalent to {@link SirixDeweyID#toBytes()} but uses lookup tables
   * and direct byte manipulation for performance.
   * 
  ...
  /**
   * Returns the number of bits needed to encode a single division value. Uses branchless comparison
   * pattern for better branch prediction.
   */
  public static int getBitsForValue(int value) {
    // Binary search pattern reduces branches from 5 to ~2-3
    if (value < TIER_1_MAX) {
      return value < TIER_0_MAX
          ? TIER_0_BITS
          : TIER_1_BITS;
    } else {
      if (value < TIER_3_MAX) {
        return value < TIER_2_MAX
            ? TIER_2_BITS
            : TIER_3_BITS;
      } else {
        return TIER_4_BITS;
  ...
  /**
   * Encodes a single division value starting at the given bit index. Returns the bit index after
   * encoding.
   */
  ...
  /**
   * Tier 0: 1-bit prefix "0" + 7-bit suffix = 8 bits total Byte layout: [0|suffix(7 bits)] where
   * suffix = value + 1
   */
  ...
  /**
   * Tier 1: 2-bit prefix "10" + 14-bit suffix = 16 bits total Prefix "10" = 0b10xxxxxx xxxxxxxx
  ...
  /**
   * Tier 2: 3-bit prefix "110" + 21-bit suffix = 24 bits total Suffix = value - 16511 (0 to 2097151)
   */
  ...
  /**
   * Tier 3: 4-bit prefix "1110" + 28-bit suffix = 32 bits total Suffix = value - 2113663 (0 to
   * 268435455)
   */
  ...
  /**
   * Tier 4: 4-bit prefix "1111" + 31-bit suffix = 35 bits total Suffix = value - 270549119 (0 to
   * Integer.MAX_VALUE - 270549119)
   * 
  ...
  /**
   * Writes a specified number of bits to the buffer starting at the given bit index. Used for
   * unaligned Tier 4 encoding.
   */
  ...
  /**
   * Returns the thread-local buffer for zero-allocation encoding. Caller must copy the result if
   * retention is needed.
   */
